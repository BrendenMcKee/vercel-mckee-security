...
- **Alarm contact lists (caller ID) come from Lanvac.** The full Excel export is in hand. A one-time importer loads each customer's contacts and passcodes into the portal quietly (no emails during seeding). After cutover, a list save still emails McKee and the customer. The same save writes the list to Lanvac over their API so staff do not retype it in the remote-desktop app. If the API call fails, the emails still go and someone updates Lanvac by hand as today. When each customer later activates, the invitation asks them to review their alarm contact list.
...
7. **The Lanvac contact-list export (done) and API docs.** Full Excel is in hand. When you are ready, share the API docs (no keys) so a portal list save can update the station. Emails stay. This does not block QuickBooks work.
