...
**Last updated:** 2026-08-16 (live file is McKee Security Live.QBW; PORTAL-TEST copy exists; sales receipts on 4000 with classes)
...
- **Alarm contact lists (caller ID) come from Lanvac.** The Excel export (dealer 10638) is the seed (people rows only, plus each CODE and exact CITY onto the profile). After cutover, a list save still emails McKee and the customer. The same save writes the people list to Lanvac (`POST /api/EmergencyContact/fullupdate` with dealer 10638, the WinLinks password, that customer's CODE, `usePoliceNumbers: true`, and the account CITY). Police/fire/ambulance stay station-owned. If the API call fails, the emails still go and someone updates Lanvac by hand as today. When each customer later activates, the invitation asks them to review their alarm contact list.
...
7. **The Lanvac contact-list export and API (done).** Full Excel is in hand. API contract ingested 2026-08-16 (dealer 10638 + WinLinks password in the JSON body; write `POST /api/EmergencyContact/fullupdate`). Emails stay. Confirm a test account with Lanvac before the first live write. This does not block QuickBooks work.
...
Everything else in Phase 8 is built and tested on our side. These sixteen items are the complete list of stakeholder inputs. **Items 1, 2, and 3 block the Windows install.** Cloud work (database tables, Accounting tab, import screens) can start as soon as you want; it does not touch QuickBooks.
