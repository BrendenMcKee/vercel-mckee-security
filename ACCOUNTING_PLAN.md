...
**Last updated:** 2026-08-23 (Client email is paused until go-live. 8A cloud routes already shipped. Empty mirrors until the Windows bridge runs. No posting. R54 is done. **Next: multi-site slices 1 and 2** (`docs/MULTI_SITE_ACCOUNTS.md`, R53), then an audit, then later R53 slices, then the CUA portal test (`docs/PORTAL_CUA_TEST.md`), then `qb-bridge/` on PORTAL-TEST, then import with a human grouping pass, then the Billing-tab `GO LIVE` flip only after grouping is signed off and the live QuickBooks file is connected.)
