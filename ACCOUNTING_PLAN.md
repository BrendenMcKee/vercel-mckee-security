...
**Last updated:** 2026-08-22 (Client email is paused until go-live. 8A cloud routes already shipped. Empty mirrors until the Windows bridge runs. No posting. **Next: Lanvac station layer** (`docs/LANVAC_STATION.md`, R54), then multi-site (`docs/MULTI_SITE_ACCOUNTS.md`, R53), then the CUA portal test (`docs/PORTAL_CUA_TEST.md`), then `qb-bridge/` on PORTAL-TEST, then import with a human grouping pass, then the Billing-tab `GO LIVE` flip only after grouping is signed off and the live QuickBooks file is connected.)
