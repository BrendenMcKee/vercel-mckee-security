...
| Portal mapping | People → `caller_id_contacts`. CODE → `profiles.lanvac_account_code` (normalized to `O5985` form). CITY → `profiles.lanvac_city` (live directory string only). Both are required in the admin UI when the client has security monitoring; they appear under Plan and when adding monitoring later. Police/fire/ambulance are not stored as contacts; they are looked up from the directory for display (admin station card + client alarm-contact card). `sort_order` → `E1`/`E2`/… at API write. Passcode → `note` `PW:…`. |
