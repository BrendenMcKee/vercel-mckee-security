...
| Station writes (2026-08-22) | Portal writes live on `O5985` only. Account OnTest/OffTest 200 and fast. Zone OnTest 200; GET `onTest` can lag; description is padded with a trailing `+` while on test. Zone/Account OffTest is 500 if already off. Unused zone 7 `BUR` create / description PUT / delete 200 in ~10s. Carbon monoxide type writes still refused. Do not PUT existing live zones (GET has no delay/call-list/codes). Site restored to the seven-zone table. `fullupdate` still off. |
