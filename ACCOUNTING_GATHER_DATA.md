...
| IP / rate limits | No allowlist block from this PC. Wrong-password POST from this workstation returns 401 (2026-08-22), so the host is reachable. Rate limits unknown. Vercel serverless egress is still unproven (same leftover as `fullupdate`). |
| Station reads (2026-08-22) | `POST /api/Account`, `/api/Zone`, `/api/Historic` on `O5985` all 200 from this PC with dealer env. `panelType` empty, `isDisabled` false, `language` `en`. Seven zones (1-6 and 9; no 7/8). GET `zoneType` is English (`FIRE`, `BURGLAR`, `LOW TEMPERATURE`, `CARBON MONOXIDE`), not the 3-char write enum. Historic `elementsPerPage` 50 works. Dates `MM-DD-YYYY HH:mm:ss`. Page 1 can be Mobi/email ops (`-X0019`, `-X0071`, `-X0070` file viewed); fire alarm/restore is signal `110011`. On-test history uses `-X0076` / `-X0030` / `-X0043`. Restore table and color rules: `docs/LANVAC_STATION.md`. Raw JSON in gitignored `website/.lanvac-o5985/`. |
| Station writes (2026-08-22, UI close-out 2026-08-23) | Portal writes live on `O5985` only. **On-test is the whole account** (`Account/OnTest` / `OffTest`). We do not put a single zone on test. Account GET has no on-test field; zone GET stays off while the account is on test; `on_test_until` is our SoR. Immediate OffTest can 500; retry once (200 if it was still on, 500 if already off). Unused zone 7 `BUR` create / description PUT / delete 200 in ~10s. Carbon monoxide type writes still refused. Staff add/edit is number, type, and description. Writes always send McKee defaults (`delay = 1`, caller-ID list, no extra notify, no codes), including on pulled fire / burglar / low-temp. Add refuses a number already on file. Site restored to the seven-zone table. `fullupdate` still off. |
| Portal mapping | People → `caller_id_contacts`. CODE → `profiles.lanvac_account_code` (normalized to `O5985` form). CITY → `profiles.lanvac_city` (live Ontario directory string only). Both are required in the admin UI when the client has security monitoring; they appear under Plan and when adding monitoring later. Police/fire/ambulance are not stored as contacts; they are looked up from the directory for display (admin station card + client card, below the people list). `sort_order` → `E1`/`E2`/… at API write. Passcode → `note` `PW:…`. Pre-API portal work closed 2026-08-16; write is later. |
| Thank-you email | Sent 2026-08-16 to Stephanos (`stephanos@lanvac.com`), cc Adrien (`adrien@prog1.ca`). |
| Vercel env | **Set 2026-08-16:** `LANVAC_API_BASE` (`https://lanvac.mobi:8843`), `LANVAC_DEALER_ACCOUNT` (`10638`), `LANVAC_DEALER_PASSWORD` (WinLinks). Server-only, never `NEXT_PUBLIC_`. Adding them does not turn on writes. |
| CITY spellings | Official write key is the live directory string from `GET /EmergencyContact/emergencynumbers` (3964 unique `city` values, pulled 2026-08-16). Excel aliases (`HALIBURTON`, `HALIBURTON ON`, `ALGONUIN HIGHLANDS`) are **not** in that directory and must be mapped before write. McKee frequency (mapped): `Haliburton - On` (294), `Minden - ON` (139), `Algonquin Highlands - ON` (71), `West Guilford - ON` (67), then the rest. Admin dropdown is Ontario only (~806), McKee-frequency first (`website/src/lib/portal/lanvac-cities.ts`). |
