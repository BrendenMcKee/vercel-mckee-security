...
| Station writes (2026-08-22) | Portal writes live on `O5985` only. **On-test is the whole account** (`Account/OnTest` / `OffTest`). We do not put a single zone on test. Account GET has no on-test field; zone GET stays off while the account is on test; `on_test_until` is our SoR. Immediate OffTest can 500; retry once (200 if it was still on, 500 if already off). Unused zone 7 `BUR` create / description PUT / delete 200 in ~10s. Carbon monoxide type writes still refused. Do not PUT existing live zones (GET has no delay/call-list/codes). Site restored to the seven-zone table. `fullupdate` still off. |
