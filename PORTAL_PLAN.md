...
| R54 | Lanvac station layer: zones, Historic signals, panel status, on/off test | Stakeholder 2026-08-22. **Read UI and O5985-gated writes / on-test shipped and live-tested 2026-08-22. Next is R53.** One CODE = one site = one zone list. Admin fetch/create/edit/delete zones and account or per-zone on-test. Client is read-only on zones/signals/panel except **account-level** on-test (Account admin after R53; today the one login). Historic is a paged log, not a live stream. Status chip is last-known (disabled / on test / last signal / else). Writes stay `O5985` until you say go. Carbon monoxide type writes stay refused. Do not PUT a pulled live zone (GET has no delay/call list). GET `onTest` can lag; trailing `+` means on test. OffTest is 500 if already off. Reads allowed for any CODE on a portal profile. Actions take `profileId` from day one. Portal `devices` stay a separate replacement list. Never call `/Account/status` or `/Account/new`. Locked model: [`docs/LANVAC_STATION.md`](./docs/LANVAC_STATION.md). OpenAPI: `https://lanvac.mobi:8843/swagger/v1/swagger.json` |
| R55 | Technician mobile app + Alarmnet 360 (after GO LIVE) | Stakeholder 2026-08-22. **Future. Do not build in R54 or R53.** Expo app (offline), TSheets clock in/out, on/off test, Historic, shared per-site retrieval agent (not compaction), tagged job photos, battery/device handoff to admin. After production: Honeywell Alarmnet 360 API ingest, then portal writes Lanvac so the office does not re-key zones. Web stays admin + client |
...
- [x] **Lanvac station writes / on-test (R54 remainder)** (2026-08-22). `lanvac_zone_write` (admin SELECT only). O5985 gate. Admin zone create/update/delete with reason + staff email. Admin account and per-zone on-test. Client account-level on-test with 120s cooldown. Carbon monoxide type writes refused. Alerts badge while on test. Live O5985 probe: BUR create/update/delete proven on unused zone 7; GET `onTest` lags (`+` suffix); OffTest 500 if already off; pulled live zones are not PUT. `fullupdate` still off.
...
| Date | Milestone |
|------|-----------|
| 2026-08-22 | **R54 writes live-tested on O5985 and audited.** Account and zone OnTest/OffTest, unused zone 7 BUR create/update/delete, then restore. GET `onTest` lags; treat trailing `+` as on test; 45s pull grace; OffTest 500 if already off; 20s write timeout; do not PUT pulled live zones. `-X0070` is Mobi file viewed (ops); `-X0043` is zone on-test email. Next is R53. Do not type `GO LIVE`. |
