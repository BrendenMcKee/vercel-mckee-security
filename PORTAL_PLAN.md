...
**Scope:** Handover Sections 1 to 22 (core portal, Phases 0 to 7) **plus the QuickBooks accounting rail and accounting MCP server (handover Section 23 Scopes B and C), pulled into Track 1 as Phase 8 by stakeholder decision R37 (2026-07-07)**: automated monitoring billing is only complete when the money also lands in the books without hand-keying. Architecture and logistics in Section 9.5; the implementation checklist is Phase 8 in Section 10. **VoIP phone service joined Track 1 as Phase 9 on 2026-07-18 (R42)**: a third service type on the exact rails monitoring proved out, built and shipped **before** Phase 8 so the accounting automation is designed against the full service catalog from day one (Section 9.6).
**Build order (decided 2026-07-05, amended 2026-07-07, 2026-07-18, and 2026-08-22):** Track 1 is the core portal **plus accounting automation**: Phases 0 to 5, Phase 7 hardening, then Phase 9 (VoIP service rail; executed 2026-07-18, before Phase 8 by stakeholder direction), then Phase 8 (QuickBooks bridge, task queue, Stripe-to-books posting, accounting MCP server). **Before the Windows bridge or any real client import:** Lanvac station layer (R54 / [`docs/LANVAC_STATION.md`](./docs/LANVAC_STATION.md)), then multi-site accounts (R53 / [`docs/MULTI_SITE_ACCOUNTS.md`](./docs/MULTI_SITE_ACCOUNTS.md)), then the CUA playbook ([`docs/PORTAL_CUA_TEST.md`](./docs/PORTAL_CUA_TEST.md)). Phase 8A cloud routes already shipped; the Windows `qb-bridge/` sitting waits on that CUA gate. The Stripe go-live (test mode to live mode) should land together with Phase 8C so every real payment posts to QuickBooks from day one. Track 2 is the camera cloud backup service: Phases 6A/6B, started **after** Track 1 is live. The camera design (Sections 9.2/9.3) is fully specified and stays in this plan, but nothing in Track 1 depends on it. Technician mobile + Alarmnet 360 are **R55**, after GO LIVE.
...
**Next product work:** Rest of R53 slice 4 (Appoint account admin, attach/move a site on the Account card), then the rest of slices 5–6 (client switcher / sites list / People, emails). Staff More info help, create-form grouping hint, and client header site · CODE shipped 2026-09-12. Grouping board shipped 2026-09-07. Slice 4 first increment shipped 2026-08-30. Clients / Account / Alerts UI polish signed off 2026-09-05 (list = two red buttons, form = toggle, this-site first, filter amber, Alerts clock = Security clock, House writes read-only). Slices 1–3 shipped. Hosted two-site fixture (do not delete): McKee account, Bunkie `O5985` + House `O4964`. Then the CUA playbook ([`docs/PORTAL_CUA_TEST.md`](./docs/PORTAL_CUA_TEST.md)) before the Windows bridge or real import. R54 read/write is done. **R54b** (show portal on-test actor) and **R54c** (optional staff Lanvac passwords) wait until after remaining R53 slices. Client mail stays off (R52 / 9.5.5C). When a slice changes UI or schema, update those docs in the same commit so implementation is never working from a stale page.
...
| R14 | Device expiry alerts | `expiry_alerted_at` timestamp on `devices`, cleared when the install date changes, so the nightly cron emails once per expiry event instead of every night. Stamp only when the admin alert sent **and** the client notice is done or not required. While client mail is paused (R52), do not stamp if the site has an email: the first run after GO LIVE still notifies the client |
...
| R22 | Dual billing rails: autopay + manual | Every paid service is `billing_method = 'stripe'` (autopay via Checkout/webhooks) or `'manual'` (legacy e-transfer/cheque/cash). Manual services carry `monthly_amount_cents` + `next_due_on`; a daily cron reminds the client before due and when overdue (R14-style `due_alerted_at` guard, once per cycle) and sends the admin a collections digest so no legacy payment is missed. **Client reminders stay off until the 8C go-live flip (R52 / 9.5.5C)**; the digest still sends. Payments are recorded in an append-only `manual_payments` ledger that advances `next_due_on`. The admin Billing tab (7.2/7.3) is the collections console; failed Stripe payments land on the same board |
...
| R42 | VoIP phone service is a first-class portal service (Phase 9) | Stakeholder 2026-07-18: VoIP joins monitoring on the full billing rails (Stripe autopay + manual ledger + reminders + collections) so it too can take automatic payments. `service_type` gains `'voip'` with two plans: `residential` and `professional`. Only two customers exist today, so no bulk import; the stakeholder enters both manually, **deferred to the Stripe go-live by stakeholder choice**. VoIP bills **monthly** on both rails, unlike monitoring's annual invoicing. Built before Phase 8 so accounting automation covers the whole catalog. **Per-line billing and the "Business" label were superseded by R50 (2026-08-13)** |
| R43 | Pre-launch Camera Cloud Backup interest is explicit, revocable consent | Stakeholder 2026-07-18: clients without cloud backup see a concise benefit card at the very bottom of their dashboard, below billing and all active-account information. "Notify Me When Available" records the profile's existing email in `cloud_backup_interest`; the consent sentence sits beside the button, the client can leave the list at any time, and no row exists without consent. Admin client detail shows the opt-in date and email. This is an availability-update list only, not a service assignment or purchase; R21 remains unchanged |
| R44 | Camera Cloud Backup admin controls remain visible but locked until Track 2 | Stakeholder 2026-07-18: both admin service-assignment surfaces keep a separate, grayed-out Camera Cloud Backup template marked **"In Development"**. It previews the established 7-/30-/90-day retention choices (R19; these map to the current storage-class design) and planned-copy also mentions a future 180-day window (not an assignable tier), but the select is disabled and normal create-client/add-service server actions reject cloud assignment. `SERVICE_LAUNCH_STATUS.cloud_backup` is the single application gate. It flips to available only after Phase 6A/6B launch gates pass, so a visual placeholder can never be mistaken for a working service |
| R45 | Caller ID and devices are security-monitoring surfaces | Stakeholder 2026-08-13: the alarm contact list and device-tracking sections appear only when the client has a current (not cancelled) monitoring service, or when historic rows already exist (contacts, change history, or devices). VoIP-only and cloud-only accounts do not see those sections. Selecting a monitoring plan on the create-client form dynamically opens optional caller ID and device seeders so the first visit to client detail is not required to start that data |
| R46 | Wrong-door between the two portals | Stakeholder 2026-08-13, amended 2026-08-23: an active admin who signs in at `/user-dashboard` sees a staff-only message and a link to `/admin-dashboard`. A signed-in **client** who opens `/admin-dashboard` (or any staff URL) sees client chrome and a link to `/user-dashboard`, not a 404. Unused-service promo cards are never shown to staff. Client and admin sign-in chrome stay visually distinct (client: shield + brand red; staff: amber console + "Staff only") so the two doors cannot be mistaken for each other |
| R47 | Client portal is tabbed; sign-in email is locked | Stakeholder 2026-08-13, layout revised 2026-08-23: `/user-dashboard` has Dashboard / Security / Settings / Alerts. Dashboard is billing-first (payments, then compact service cards). Security appears only with current monitoring or leftover station/contact/equipment rows; it holds zones, Historic, the alarm contact list, and equipment. Payment and missing-contact banners stay on Dashboard and are mirrored on Alerts. Alerts uses the same always-on count badge as admin (green `0` / red count). Settings lets the client change phone, service address, and password while signed in (password change requires the current password). The profile email is shown disabled: it is the sign-in identity (Google OAuth cannot be re-linked by rewriting `auth.users.email`). McKee is emailed on any client-made account change so QuickBooks can be updated by hand. Clients still have no UPDATE RLS on `profiles`; the settings action writes `phone`/`address` via service role |
| R48 | Admin-action mail is wider, shows the full caller ID list, and uses inbox emojis | Stakeholder 2026-08-13: branded email cards are 760px so desktop admin mail is not a skinny column. Caller ID admin (and matching client) mail shows the **entire** list: added green, removed red, unchanged gray/white. Admin-action subjects use a leading emoji (`📞` caller ID, `⚠️` card failed, `🔋` device due, `📋` collections, `✏️`/`🔐` account changes) so the inbox scan is obvious. Client-facing mail stays emoji-free |
| R49 | Monitoring history starts on the day they were first monitored, not the day they joined the portal | Stakeholder 2026-08-13: invitation date and activation date are portal-access events. Every monitoring service also stores `started_on` (the day that client actually began being monitored, which for a legacy account can be years earlier). Create-client and add-service collect it; the QuickBooks seed infers it from the earliest monitoring invoice on that customer. Year-over-year monitoring profitability cannot be built without this. Distinct from `next_due_on` (billing) and `profiles.created_at` (when the portal row was typed in) |
| R50 | VoIP bills once per system from the 3.12 rate card, never per line | Stakeholder 2026-08-13, company knowledge 3.12. One subscription, one line item, quantity 1. The client line names the service and what it covers (number count, seat count) and carries the total. Base and add-ons are never separate client lines. `monthly_pre_tax = base + (4.99 x additional_numbers) + (24.99 x additional_seats)`, then HST 13% on that sum once. Commercial (`professional`, displayed **Commercial**) base $59.99 includes 1 number + 1 seat. Residential base $34.99 includes 1 number + 1 seat and has **no seat add-on**. Additional numbers $4.99 on both plans. Additional seats $24.99, commercial only. Number port fee $49.99 one-time per number ported, never on the subscription. Phones and handsets add nothing (Haliburton Automotive: three handsets, one number, $59.99 total). Recurring is fully separate from installation: never in a project total, payment threshold, payment split, or one-time install invoice. Columns: `number_count`, `seat_count`, `port_count` replace `line_count`. Internal BrightPBX cost is $5.95 per seat per month; DID cost treated as $0.00 unconfirmed. Cost never surfaces on a client document |
| R51 | Phase 8 develops against a named copy of the live company file, never the live books | Stakeholder 2026-08-13. The live QuickBooks company file keeps running the business (invoices, deposits, TSheets / QuickBooks Time Web Connector). 8A/8B and the bulk import rehearsal run against a backup restored as a separately named company. The Intuit sample company is only a last-resort SDK smoke test if the copy is not ready. The live file is first opened by the bridge at 8C, after the bookkeeper mapping. TSheets stays authorized on the live file only. The bridge uses the Desktop SDK, not Web Connector, so it does not share TSheets' connector. `qb_bridges.mode` is `sandbox` or `live`; sandbox postings never count as posted to the live books. See 9.5.7 |
| R52 | Client-facing portal mail stays off until an explicit go-live flip | Stakeholder 2026-08-22. Importing real emails and due dates must not start automated onboarding. `portal_settings.client_mail_enabled` defaults false. Invitations, payment reminders/receipts, caller-ID client notices, Stripe success mail, and device-expiry client notices no-op until an admin types `GO LIVE` on the Billing tab. That flip is an 8C human step after **the station layer (R54), multi-site (R53), and the CUA test**, then import is checked, grouping is signed off, the portal is trusted, and the bridge is on the live QuickBooks file. Staff mail stays on. See 9.5.5C |
| R53 | Multi-site accounts: one account, many sites, extra logins | Stakeholder 2026-08-22. **Slices 1–3 shipped** (1–2 audited 2026-08-28; last-owner revoke + this-site delete copy 2026-08-29). Slice 4 first increment shipped 2026-08-30. Clients / Account / Alerts UI polish signed off 2026-09-05. Grouping board shipped 2026-09-07. Staff More info help, create-form grouping hint, and client header site · CODE shipped 2026-09-12. Remaining: Appoint / attach, switcher, emails. Today's `profiles` row stays one **site** (one Lanvac CODE, one caller list, one zone list, one monitoring bill, optional VoIP, one Stripe customer, one QB customer). Add `accounts` + `account_members`. Client-facing roles: **Account admin** (schema `owner`) and **Member**. Extra people do not get `profiles.user_id`; access is membership only (`private.can_access_profile`). Drop global unique `profiles.email` when this ships. Single-site clients keep today's dashboard (no switcher, no People list). Two Clients-tab buttons: New client = one new account + one site; Add site to an account = pick account then site form. No org checkbox. Import never auto-merges: AI suggests, human accepts. `auto_onboard` (default true) turns off on a second site and mutes onboarding mail only. Human grouping sign-off (`portal_settings.org_grouping_reviewed_at`) is a GO LIVE gate; empty-queue sign-off is allowed. Appoint account admin before bills are due. CUA playbook is the last gate before the Windows bridge or real import. Locked model: [`docs/MULTI_SITE_ACCOUNTS.md`](./docs/MULTI_SITE_ACCOUNTS.md). Test playbook: [`docs/PORTAL_CUA_TEST.md`](./docs/PORTAL_CUA_TEST.md). Implementation: two slices then an audit. Do not start the Windows bridge, real import, or type `GO LIVE` until R54, this work, and the CUA report are done |
| R54 | Lanvac station layer: zones, Historic signals, panel status, on/off test | Stakeholder 2026-08-22. **Read UI and O5985-gated writes / on-test shipped and live-tested 2026-08-22. Next product work is remaining R53.** One CODE = one site = one zone list. Admin fetch/create/edit/delete zones. **On-test is the whole account only** (admin and client Account admin after R53; today the one login). We do not put a single zone on test. Historic is a paged log, not a live stream. Status chip is last-known (disabled / on test / last signal / else). Writes stay `O5985` until you say go. Carbon monoxide type writes stay refused. Staff zone add/edit is number, type, and description only. Writes always send McKee defaults (`delay = 1`, caller-ID list, no extra notify, no codes), including on pulled fire / burglar / low-temp zones. Add refuses a number already on file. Account GET has no on-test field; `on_test_until` is the SoR. OffTest is 500 if already off. Reads allowed for any CODE on a portal profile. Actions take `profileId` from day one. Portal `devices` stay a separate replacement list. Never call `/Account/status` or `/Account/new`. Locked model: [`docs/LANVAC_STATION.md`](./docs/LANVAC_STATION.md). OpenAPI: `https://lanvac.mobi:8843/swagger/v1/swagger.json` |
| R54b | Show portal station actor on Historic / on-test | Stakeholder 2026-09-05. **Planned. Do not build before remaining R53 slice 4.** `lanvac_station_events` already stores `actor_user_id`, `actor_email`, CODE, and site. Historic from Lanvac only shows the shared dealer login. Overlay portal events (“Made by {name} · {CODE} · {account}”). Grant client SELECT on own events via `can_access_profile`. Do not invent a second audit table. Locked notes: [`docs/LANVAC_STATION.md`](./docs/LANVAC_STATION.md) |
| R54c | Optional per-staff Lanvac API password | Stakeholder 2026-09-05. **Later, after R54b.** Username stays `LANVAC_DEALER_ACCOUNT`. Staff may store their own WinLinks password so Mobi Historic names the technician. Fallback is the Vercel env password. Clients never get a password field; client writes stay on the env login. Encrypt at rest. Confirm with Lanvac that password-per-operator is how Historic labels the user before storing secrets. Not required to know who clicked in the portal. |
| R55 | Technician mobile app + Alarmnet 360 (after GO LIVE) | Stakeholder 2026-08-22. **Future. Do not build in R54 or R53.** Expo app (offline), TSheets clock in/out, **account-level** on/off test (never a single zone), Historic, shared per-site retrieval agent (not compaction), tagged job photos, battery/device handoff to admin. After production: Honeywell Alarmnet 360 API ingest, then portal writes Lanvac so the office does not re-key zones. Web stays admin + client |
| R24 | Admin caller ID changes require recorded authorization + client notification | Caller ID lists drive emergency response, so an unauthorized or missed change is a security exposure. Reconciliation model (stakeholder, 2026-07-05): **(1)** a client-made change is self-authorizing (authenticated session = proof). **(2)** An admin-made change cannot be saved without an **authorization method** (`client_email` preferred and nudged by the UI, `client_verbal`, `client_in_person`, `mckee_initiated` for corrections) and a **required reason note** referencing the request (e.g. "Client emailed 2026-07-05 asking to remove former employee"). **(3)** The client is **always emailed** when an admin changes their list (exact diff, recorded reason, dispute path) **once client mail is live (R52)**; while paused the send is held and `client_notified_at` is not stamped. **(4)** History is **append-only and immutable** (no UPDATE/DELETE policies), so the record of who/when/what/why/how can never be rewritten. Written (email) requests are the preferred evidence; verbal changes are allowed but explicitly flagged, and the client notification acts as the dispute backstop either way |
...
└── src/
    ...
    │   │   │   └── page.tsx                    # Dashboard / Security / Settings / Alerts (R47). Cards render inline via PortalCard
    │   │   └── account/
    ...
    │   ├── admin-billing.tsx
    │   ├── client-mail-gate.tsx / client-mail-paused-banner.tsx  # 9.5.5C
    ...
    │   ├── auth.ts
    │   ├── invitations.ts
    │   ├── client-mail.ts / client-mail-phrase.ts  # 9.5.5C fail-closed read + GO LIVE phrase
    │   ├── actions/
    ...
    │   │   ├── devices.ts
    │   │   ├── payments.ts
    │   │   ├── settings.ts                     # client-mail go-live flip
...
Browser -> proxy.ts (portal matcher only: refresh session cookies)
        ...
        -> (admin-portal)/layout.tsx  getClaims() + profiles.role check -> client: wrong-door (R46); other non-admins: 404
...
**Current identity (R53 slices 1–2):** one `profiles` row is the **site**. `accounts` + `account_members` hold the login. Client RLS is `private.can_access_profile` (membership only). Extra members do not get `profiles.user_id`. Single-site clients keep today's dashboard (no switcher yet).
...
create type public.service_type     as enum ('monitoring', 'cloud_backup', 'voip');  -- 'voip' added 2026-07-18 (R42)
...
**VoIP plans** (stakeholder 2026-07-18, R42; rate card locked 2026-08-13, R50 / company knowledge 3.12; **per month plus tax, billed monthly** on both rails; **once per system**, never per phone / number / handset):

| Tier key | Displayed as | Base system (pre-tax) | Includes | Add-ons |
|----------|--------------|-----------------------|----------|---------|
| `residential` | Residential Unlimited Canada-Wide | $34.99 | 1 number + 1 seat | Additional numbers $4.99. **No seat add-on** |
| `professional` | Commercial | $59.99 | 1 number + 1 seat | Additional numbers $4.99. Additional seats $24.99 |

Derivation: `monthly_pre_tax = base + (4.99 x additional_numbers) + (24.99 x additional_seats)`, then HST 13% on that sum once. The client document is **one line** at that total (the line names the service and coverage). Number port fee is $49.99 one-time per number ported and never appears on the subscription. Recurring is never folded into an installation invoice, project total, payment threshold, or payment split.

Worked figures (must stay valid in `billing.ts` / `voip-check.mjs`): residential 1+1 = $34.99 / $39.54 with HST; commercial 1+1 = $59.99 / $67.79; commercial 2 numbers 1 seat (Vision Care Centre) = $64.98 / $73.43; commercial 1 number 3 seats = $109.97 / $124.27.

VoIP prices live in `billing.ts` (`VOIP_MONTHLY_CENTS` is the base; `voipMonthlyCents()` / `serviceMonthlyCents()` are the billed total). Assigning or changing a VoIP plan or its numbers/seats prefills `monthly_amount_cents` as that derived total. Stripe catalog prices are the two bases; configurations above the base reuse the same product at a matching monthly CAD price, quantity 1.
...
| `email` | text | Site contact. **R53 slices 1–2 dropped the global unique** so two sites can share a contact email; login identity is the member email / Auth user. **Required in the create/edit admin forms** (invitation, Stripe receipts, reminders, sign-in identity). Column stays nullable only for any pre-existing row. Client-locked in Settings (R47); admin may still edit the contact/invite email without changing Auth |
...
| `lanvac_account_code` | text | nullable in the column; unique when set. Lanvac account CODE (`O5985`). Pattern `^[0-9A-Za-z]{1,2}[0-9A-Fa-f]{4}$`. Admin-only. **Required in the UI when the client has security monitoring.** Input is normalized (dashes/spaces stripped; four hex digits get a leading `O`). |
| `lanvac_city` | text | nullable in the column; 1..240. Must be an exact Ontario key from the live Lanvac emergency-numbers directory (`policeNumbersCity`). Admin dropdown: McKee-frequency first, then other Ontario cities only (~806; no other provinces) (`lanvac-cities.ts`). Guessed from service address when possible; admin can override. Police/fire/ambulance stay station-owned and are shown read-only from that city. |
...
| `tier` | text | not null. CHECK: `monitoring` in ('landline','cellular','cellular_tc','cellular_tc_home'); `cloud_backup` in ('7day','30day','90day'); `voip` in ('residential','professional') (R42) |
...
| `billing_interval` | billing_interval | not null default `'monthly'`; **locked** per service: monitoring is always `'annual'` (DB CHECK + admin UI, no monthly option); VoIP is always `'monthly'` (DB CHECK). Manual-rail `next_due_on` defaults to today on setup; recording a payment advances it one interval |
| `number_count` | integer | not null default 1, CHECK 1..100 (R50). VoIP: how many numbers the system includes (base includes 1). Non-VoIP keep 1 |
| `seat_count` | integer | not null default 1, CHECK 1..100 (R50). VoIP commercial: user seats (base includes 1). Residential VoIP is forced to 1 (DB CHECK + app). Non-VoIP keep 1 |
| `port_count` | integer | not null default 0, CHECK 0..100 and `<= number_count` (R50). How many of those numbers are being ported. Drives the one-time $49.99 port fee only. Never part of `monthly_amount_cents` |
| `port_fee_charged_count` | integer | not null default 0, CHECK 0..100. How many of `port_count` have already had the one-time fee recorded. Charge only the remainder. A second click cannot double-bill |
| `monthly_amount_cents` | integer | nullable; the **monthly pre-tax total** (an annual invoice = 12x + tax; for VoIP this is the 3.12 derived total, one figure). Required when `billing_method='manual'` (reminder emails and the Billing tab quote it) |
| `started_on` | date | nullable in the schema, **required in the admin UI when the service is monitoring** (R49). The day this client actually began the service (legacy: first monitored; new: start of this contract). Not invitation, not activation, not `created_at`. Seed infers from the earliest matching QB invoice |
...
**`cloud_backup_interest`** (R43; pre-launch, consent-only contact list)

| Column | Type | Constraints |
|--------|------|-------------|
| `profile_id` | uuid | PK + FK profiles `on delete cascade`; one active opt-in per client |
| `email` | text | not null, 3..320 characters; the server action copies `profiles.email`, and RLS independently requires an exact case-insensitive match |
| `consent_version` | text | not null default/check `'cloud_backup_updates_v1'`; records the copy agreed to |
| `consented_at` | timestamptz | not null default `now()` |

A row exists **only while consent is active**. Joining inserts the row; "Remove me from this list" deletes it. Clients never supply an arbitrary address, and this table does not assign a service, charge a card, or imply a contract.
...
| `label` | text | not null, length 1..80 in the schema; **app max 30** to match Lanvac `name` |
| `passcode` | text | nullable in schema (pre-existing rows), length 1..40 in the schema; **app max 21** so `PW:{passcode}` fits Lanvac `note` (24). Required by the app for every contact. Displayed name-first: call order, label, phone, passcode |
...
**`devices`** (reshaped by R31: open admin-managed list, no fixed pair; R35 adds categories)

| Column | Type | Constraints |
|--------|------|-------------|
| `id` | uuid | PK |
| `profile_id` | uuid | FK profiles, not null |
| `label` | text | not null, 1..80 chars (CHECK) — free-text name (`Hallway smoke 1`); presets are starting points only |
| `category` | text | not null, default `'other'`, CHECK in (`system_battery`, `device_battery`, `detector`, `other`) — the only filter. `device_battery` covers every wireless sensor battery, including a smoke/CO battery. `wireless_device` was folded into `device_battery` (2026-08-13) |
...
Expiry is computed, not stored: `installed_on + lifetime_years`. Accounts start with zero devices; admins type a name or pick a common preset (the old `device_type` enum and its unique pair constraint were dropped in migration `20260706210000`; `category` added in `20260706233000`). The Devices tab filters by category only. A wireless smoke/CO unit is two rows: the detector (`detector`) and its battery (`device_battery`).
...
Phase 8C reverse-sync deltas (9.5.5B, Migration 8C): `qb_txn_id` text nullable, **unique when set** (idempotent QB imports); `recorded_by` relaxed to nullable with CHECK (`recorded_by is not null or qb_txn_id is not null`), so only QuickBooks-recorded rows may omit the admin actor (`recorded_by_email` carries `quickbooks-sync`).
...
**`portal_settings`** (singleton ops flags; client-mail gate 9.5.5C)

| Column | Type | Constraints |
|--------|------|-------------|
| `id` | integer | PK default 1, CHECK `id = 1` (one row) |
| `client_mail_enabled` | boolean | not null default false |
| `client_mail_enabled_at` | timestamptz | nullable |
| `client_mail_enabled_by` | uuid | nullable (admin who flipped it on) |
| `updated_at` | timestamptz | not null default `now()` |

Stays false through import. The 8C Billing-tab flip is the only intended write. Admin SELECT/UPDATE. Reads fail closed.
...
**8A first migration slice (applied 2026-08-17 on hosted):** `supabase/migrations/20260817143259_qb_bridge_foundation.sql` (hosted version `20260817143259`). Tables: `qb_bridges`, `qb_customers`, `qb_invoices`, `qb_payments`, `qb_todos`. Alter: `services.started_on date`. RLS: admin SELECT on all five; no client policies; no INSERT/UPDATE/DELETE for `authenticated` (bridge routes use service role). Unique index on `qb_customers.profile_id` where not null. Types regenerated. **Cloud routes (2026-08-17):** `/api/qb/poll`, `/api/qb/report`, `/api/qb/mirror` are live in the website app. **Still not built:** remaining R53 slices (Appoint / attach, switcher, emails), CUA gate, `qb-bridge/`, Accounting tab, import UI, Lanvac `fullupdate`, `qb_tasks`. **Order:** finish R53, then CUA, then Windows bridge, then import. R53 slices 1–3 shipped; slice 4 first increment shipped 2026-08-30; grouping board shipped 2026-09-07; staff More info help, create-form grouping hint, and client header identity shipped 2026-09-12. R54 read + O5985-gated writes shipped 2026-08-22.

### 4.3 RLS policy matrix

This matrix is the live membership model (R53 slices 1–3). Client access is `private.can_access_profile`. `is_admin()` stays. Later slices add the switcher UI. Delete-Auth / per-site disable live holes were closed in the 2026-08-28 audit. Last-owner revoke and multi-site delete confirm copy shipped 2026-08-29.
...
|-------|------|----------------------|-------|
| `profiles` | none | SELECT own (`private.can_access_profile`). No INSERT/UPDATE/DELETE (name/address changes go through admin) | SELECT/INSERT/UPDATE all via `private.is_admin()`. No DELETE policy; deletion happens only through `deleteClientAction` (service role behind `requireAdmin()`, client-role targets only, confirm dialog) |
...
| `cloud_backup_interest` | none (table grant revoked) | SELECT/INSERT/DELETE own (`can_access_profile`; site-contact email may differ from the login). No UPDATE grant. A row is active consent | SELECT all via `private.is_admin()`; no admin write path |
...
| `portal_settings` | none | none | SELECT/UPDATE the singleton row (`id = 1`). No INSERT/DELETE policies. `client_mail_enabled` defaults false; the 8C `GO LIVE` flip is an admin UPDATE |
...
2. No session: render `SignIn` in place (logged-out state copy: "Securely manage your account information, cloud backups, and more." CTA "Manage Account", prominent "Continue with Google", secondary email/password form).
   - **Uninvited sign-ins are rejected at the door (stakeholder 2026-07-05).** R9 keeps OAuth signups enabled for the activation flow, so a plain Google sign-in without a **membership** briefly creates an auth user; the callback detects no `profiles.user_id` and no `account_members.user_id`, signs out, **deletes that auth user**, and returns to `SignIn` with "no account was created" copy. The activation-complete path is exempt. Password sign-ins get the same treatment client-side. The orphan screen (step 5) is the fallback for a session with no membership.
...
5. Session exists but **no membership** (orphan): show "We could not find an account for `<email>`…" with sign-out. If every accessible site is disabled: “This account is disabled,” not the orphan screen.
6. Session + an active accessible site: render dashboard sections (Section 7) for the selected site (cookie / `?site=` if authorized, else first active).
...
4. If email provided **and client mail is live (R52 / 9.5.5C)**: send invite email (Section 8) with `https://mckeesecurity.ca/account/activate?token=<raw>`. If mail is paused, or no email is on file: surface the link in the admin UI (held, not a send failure).
5. Return created client to refresh the admin table.

Resend (handover 7.3): expire open invitation, create new token, re-send only when client mail is live. Available until profile `status='active'`.
...
`(admin-portal)/layout.tsx`: `getClaims()`; then fetch own profile via user-context client. **Signed-out visitors get the shared `SignIn` screen in place** (stakeholder, 2026-07-05: admins should be able to sign in directly at `/admin-dashboard`; the Google flow returns them there via `next=/admin-dashboard`). A signed-in **client** gets the R46 wrong-door (client chrome, CTA to `/user-dashboard`). Anyone else who is not an active admin still gets the neutral not-found. Every admin server action independently calls `requireAdmin()` (fresh DB check, not JWT claims, so demotions apply immediately).
...
Global chrome: site `Header`/`Footer` stay (portal is native to the site); welcome header "Welcome, {first_name}", sign-out, then Dashboard / Security / Settings / Alerts tabs (R47). The Security tab is omitted when the account has no monitoring and no leftover station, contact, or equipment rows. Loading skeletons per card, per-section error boundaries with retry (handover 22.3).

| Section | Content | Client actions | Rules enforced |
|---------|---------|----------------|----------------|
| Dashboard · Billing & Payments | first card on Dashboard: how each active service is paid (green "card on file" / amber "card not set up yet" / e-transfer copy), amount, next payment, unified history (manual ledger + `invoice.paid`) | Stripe customer portal when a card is on file (receipts / card update; cancel and plan-change disabled, R21) | Neutral billing chrome so this card does not look like Security Monitoring |
| Dashboard · service cards | compact Security Monitoring / VoIP / Cloud Backup plan + rate + status. Monitoring links to the Security tab. Unused services stay in a dashed "Other McKee services" band below | **none** on the plan itself (read-only); unused cloud can opt into `cloud_backup_interest` | Handover 6.2 / R42 / R43 / R50. Product colors stay on the service cards; billing stays neutral |
| Security tab | shown only with current monitoring or historic station/contact/equipment rows (R45). Header explains the subscribed monitoring plan. Then zones/Historic, alarm contact list, equipment | add / edit / remove contacts and save; account-level on/off test when writes are live; Refresh now pulls Lanvac | Station readout and the full caller-ID list load only on this tab. Lanvac Account/Zone/Historic pull runs only while this tab (or the admin client **Security** tab) is open |
| Caller ID | on the Security tab. Empty list on a fresh monitoring account also raises a Dashboard / Alerts banner with a link to `?tab=security#alarm-contact-list` | add, edit (name / phone / passcode), remove, save (single save action commits the batch) | Validation: NANP format, no duplicates, cap (D6). Save triggers admin diff email |
| Equipment Maintenance | on the Security tab. Open list (label, category, install date, lifetime) | none (read-only) | Expired tiles are amber, not brand red (handover 14). Category is the filter on the admin Devices tab; names are free text |
| Payment / setup banners | unpaid service, missing alarm contacts, or autopay with no card yet. Stay at the top of Dashboard and are mirrored on Alerts | Autopay: Pay Now / Set Up Automatic Payments -> Stripe Checkout. Manual: due date + amount + payment instructions (D11), no checkout button. Add contacts opens the Security tab | Tier server-validated, success/cancel return states; banner variant driven by `billing_method` (R22) |
...
| Alerts tab | same attention items as the dashboard banners, plus expired devices and on-test; all-clear state when the count is zero | Pay Now / Add contacts / Set up card, same as Dashboard | Badge always-on (green `0` / red count), matching admin |
...
|-----|--------------|-------|
| Overview | KPI cards grouped into **Clients / Billing / Needs attention**, with icons and tone (green = healthy zero, amber = watch, red = collect). Services-by-plan pills use the shared product colors. Recent activity feed | 3 skeleton, completed in 5; visual overhaul 2026-08-13 |
| Clients | searchable table: instant text filter on name/email, filters for status, service type, tier, billing method, and overdue; sortable columns; pagination; create-client form (**monitoring requires `started_on`**, R49); row click opens client detail | 2 basic table, 3 search + filters |
| Client detail | tabbed like the console (`?tab=`). **Account:** profile, invitation, disable/delete. **Billing:** plans (assign/modify tier, cancel/restart, rails, **monitoring `started_on`**, R49), payment history, cloud interest (R43) + In Development template (R44). Each assigned service uses the shared product color **and** icon (monitoring red / VoIP teal / cloud sky). **Security:** station CODE/city, zones/Historic, account on-test, zone add/edit/delete (McKee defaults; carbon monoxide delete-only), caller ID (R23/R24) when R45 applies. **Devices:** equipment when R45 applies. Staff mail deep-links to the matching tab. Billing / Security / Devices extras load only on that tab | 3 core, 4 caller ID editor, 5 billing fields |
...
| Devices | every tracked device across all clients, soonest replacement first; category chips (R35) + due-status filters (overdue now / due within a year); rows link to client detail | added post-Phase 7 (R35) |
| Fleet | gateway health, per-site storage and margin (9.2.7) | 6A (Track 2) |
| Alerts | recent device expiry alerts, failed email sends, payment follow-ups. The Alerts tab always shows a count badge: green `0` when clear, red with the open count otherwise | 7; badge always-on 2026-08-13 |

Client-detail dates are three different events and must stay labeled as such (R49): **invitation** (`invitations.created_at`) and **activation** (`invitations.used_at` / `profiles.user_id`) are portal-access; **Monitoring since** (`services.started_on`) is the day they first began being monitored and can predate the portal by years. Create-client and add-service require `started_on` whenever a monitoring plan is assigned (same date picker as other admin dates). Cancel/restart does not overwrite `started_on`; an admin edit is the only way to change it, so year-over-year history survives a pause.
...
**Client-facing portal mail is gated (R52 / 9.5.5C).** Every client template below dispatches through `dispatchClientEmail`, which no-ops while `portal_settings.client_mail_enabled` is false (the hosted default through import). Admin templates keep using `dispatchPortalEmail`. A pause is not an `email_failure` alert. Website contact/apply/inquiry forms and Starlink rental reminders are out of this gate.
...
Caller ID diff email: added contacts styled green, removed styled red (handover 6.4), plain-text fallback included. It fires on every list change from either side (R23), names who made the change, and stays the audit trail for McKee and the customer. The same save also posts the list to Lanvac's API (D13) so staff do not retype it in the remote-desktop app. API failure does not roll back the portal save; the emails still go; RDP is the fallback. Email send failures are logged and surfaced in the admin alerts area (handover 22.3); a failed notification never rolls back the underlying save.
...
- **Model:** one Stripe Product per service type. Monitoring: one recurring Price per tier (annual CAD, 12x monthly), IDs in env vars. VoIP (R50): one Product per plan (Residential / Commercial) with a catalog monthly CAD Price at the **base** (1 number + 1 seat). Configurations above the base find or create another monthly CAD Price on the same product at the derived total. Every VoIP subscription item is **quantity 1**. One subscription, one line. The Number Port Fee is a separate one-time Price, never a subscription item. VoIP catalog prices (bases + port fee) are found or created by Stripe metadata marker (`resolveVoipCatalogPriceId`); optional `STRIPE_PRICE_VOIP_*` env vars override if set. Cloud backup: one recurring **per-camera** Price per (retention tier, resolution) cell (9 prices, matrix in 9.2.7); the subscription carries an item per cell in use with quantity = camera count. Client code never sends price IDs.
- **Checkout:** server action verifies the caller owns the `services` row and reads the **admin-assigned tier from the database** (anti-spoofing, handover 9.3), creates a subscription-mode Checkout Session with `customer` (`findOrCreateStripeCustomer`: stored `stripe_customer_id`, else an existing Stripe customer with that email, else create), `payment_method_collection: always` so the card is saved on that customer, `metadata: { profile_id, service_id, service_type, tier }`, success URL `/user-dashboard?payment=success`, cancel URL `/user-dashboard?payment=cancelled`. Deleting a portal client cancels subscriptions but **keeps** the Stripe customer so invoices stay in Stripe; recreating the same email reuses that customer. Monitoring is annual and VoIP is monthly, so they cannot share one Checkout session. After the first successful Checkout, `activateRemainingAutopay` starts every other approved autopay service on that same card and charges any uncharged VoIP port fees. The client dashboard shows one “Add your card” banner listing all outstanding items (including the port fee). A later extra port appears as its own Pay Port Fee alert and charges the saved card.
- **Plan changes and cancellation are admin-only for every service type (R21).** Same rails for monitoring and VoIP. Admin tier / VoIP numbers-or-seats change with a live subscription = server-side `subscriptions.update` swapping the single price (`proration_behavior: none`, so the new rate is on the next invoice, not a mid-cycle surprise). Stripe is updated first; if Stripe rejects, the portal row is not changed. Cancel = `cancel_at_period_end` (they stay paid through the current period; Stripe then ends the subscription, so Restart after that needs a new Checkout). Hold billing (Pause) is the other action: `pause_collection: { behavior: "void" }` plus `cancel_at_period_end: false`. Stripe keeps the subscription and stops charging; Restart resumes the same card with no client action. That is why Hold exists: Cancel is "this service is ending"; Hold is "stop charging for a bit, keep the card on file." Manual-rail Hold stops reminders the same way. Restart clears both flags. Paused services are excluded from collections, booked-revenue KPIs, and port-fee charges. Switching autopay to manual is a different action: it **immediately** `subscriptions.cancel`s (no proration) and the paid-through date becomes the next manual due. Deleting a client immediately cancels every live Stripe subscription, then erases the row. Plan and VoIP-config edits are refused while `status = cancelled` (restart first). Clients never see change or cancel controls; their only money action is Pay Now checkout for the admin-assigned service.
...
- **Webhook `/api/stripe/webhook`:** raw body via `await req.text()`, `stripe.webhooks.constructEvent` signature verification. Insert into `billing_events` first (PK conflict = already processed, return 200). Handle: `checkout.session.completed` (service -> `active`, store subscription id **and stamp `next_due_on` from the subscription's period end**; a stale session after Cancel/Pause cancels the new Stripe subscription and does not revive the service), `customer.subscription.updated` (tier/`next_due_on` sync; status from `portalStatusFromStripe`: Stripe still reports `active` while `cancel_at_period_end` or `pause_collection` is set, so those flags must win or Cancel/Pause flips back to Active), `customer.subscription.deleted` (-> `cancelled`, clears the subscription id), `invoice.paid` (renewal: advance `next_due_on`, client payment history row; falls back to subscription metadata when it outraces checkout.session.completed, but never attaches a new subscription to a cancelled/paused service; skips `kind=voip_port_fee`), `invoice.payment_failed` (-> flag + admin email). All writes via service role. **The Stripe endpoint's enabled-events list must include all five** (R33: `invoice.paid` was missing at first setup).
...
| `/api/cron/device-expiry` | via daily / manual | expired devices where `expiry_alerted_at is null` -> email admin always, email client only when mail is live (R52); stamp `expiry_alerted_at` only when the admin alert sent **and** the client notice is done or not required (R14 / 9.5.5C). While paused, do not stamp if the site has an email |
| `/api/cron/cleanup` | via daily / manual | orphan auth users >7d, invitations expired >90d unused, rate-limit counter rows >1d, spent `rental_reminders` guard rows >90d; footage links past TTL joins in Track 2 |
| `/api/cron/payment-due` | via daily / manual | manual services with `next_due_on` inside the reminder window (D11 default 7 days) or overdue: always list them on the admin collections digest (R22, 7.3). Client reminder + `due_alerted_at` stamp only when client mail is live (R52); while paused, `due_alerted_at` stays null so the first reminder after go-live is not skipped |
| `/api/cron/starlink-reminders` | via daily / manual | internal-only Starlink rental reminders: one-shot pickup-today and payment-before-pickup emails guarded by `rental_reminders`, a daily-repeating standalone escalation for any deposit owed a day or more, plus a daily action digest (late kits, deposits to refund, unrecorded payments, unpriced bookings, bookings with no unit reserving the dates, unanswered requests) that repeats until the booking is put right. Digest sections are headed by the action, banded urgent/today/soon, and the subject lists the jobs. Partial failures land in `portal_alerts`. See [docs/STARLINK-RENTAL-REMINDERS.md](./docs/STARLINK-RENTAL-REMINDERS.md) |
...
**Client-facing billing mail moves entirely to the portal (stakeholder 2026-07-07), after the 8C go-live flip (R52).** The legacy practice of emailing monitoring invoices from QuickBooks ends at Phase 8: renewal reminders, autopay receipts, payment confirmations, and migration invitations are all portal-sent (9.4, R40) once client mail is live. QuickBooks documents are **sales receipts** created by the automation after the portal has collected the money (D12, locked 2026-08-16), never customer communication. Fixed constraints from the handover (23.13) remain non-negotiable: QuickBooks Desktop Canada stays the system of record, no unrestricted AI or website writes to QuickBooks, offline-safe queue, approval gates for posting, full audit trail.

#### 9.5.1 Where each piece lives (the logistics)

The **only** component that must run on the office QuickBooks computer is a small **local bridge**. Everything else, including the MCP server, lives in the cloud on the existing stack:

```
Cursor / AI agent (staff)                 Portal admin UI
        |                                        |
        v                                        v
MCP server (Vercel route, e.g. /api/mcp) --- Cloud API (Vercel)
        |                                        |
        +----------------+-----------------------+
                         v
        Supabase: qb_tasks queue + read-only QB mirrors + audit (RLS admin-only)
                         ^
                         | outbound HTTPS polling (no inbound ports, same
                         | pattern as the camera gateways and Data Drops)
                         v
        Local bridge (Windows service on the QuickBooks PC)
                         |
                         v  qbXML via Desktop SDK 17 (QB Canada 2023 R16+ / 2024 R18+)
        QuickBooks Desktop company file (system of record)
```

Why the bridge must be local: the Intuit Desktop SDK is a Windows COM interface that talks to a running QuickBooks Desktop instance on the same machine (LAN configurations exist but add fragility). Why the MCP server must **not** be local: staff should be able to ask questions and draft work at any time, including when the office PC is off. The MCP server is **not** installed on the QuickBooks PC and is not the same program as the bridge. Reads are served from Supabase mirrors; writes become queued tasks that the bridge drains when the PC comes online (handover 23.3: "the website must not fail because QuickBooks is powered off"). The bridge authenticates outbound with a per-device secret (hash stored server-side, same model as `gateways.secret_hash`), so the office network needs zero inbound exposure.
...
- **Develop against a named copy of the live company file (R51 / 9.5.7).** The live books stay open for the business and for TSheets. 8A/8B and the bulk import run against a backup restored as a separately named company. A backup/restore keeps QuickBooks ListIDs, so links created from the copy still match the live file at 8C. The Intuit sample company is only a last-resort SDK smoke test. The live file is first opened by the bridge at 8C, after the bookkeeper mapping (D12). Sandbox postings in the copy never count as posted to the live books.
- **8A starts after D12 machine confirmation and D17 file hygiene.** Cloud work (migrations, Accounting tab, import UI) can be written in parallel; the Windows install waits until the live file is labeled and the portal-test copy exists.
...
- **Monitoring profitability is a later view, not 8C posting.** Phase 8 posts money to the books. A year-over-year monitoring P&L (revenue minus cost per monitored client, sliced by calendar year using `started_on`) needs two inputs that posting does not: every monitoring service's real start date (R49/D15) and the cost McKee pays per client by tier (D16), stored later in a dated rate table in the Starlink `unit_costs` pattern. Gather those before treating Phase 8 as "profitability-complete." Do not invent the cost table to unblock 8A or 8C.

#### 9.5.4 Existing-customer onboarding and duplicate prevention (R38)

Most portal onboarding for the foreseeable future is **existing customers moving off legacy payment methods**, and every one of them already exists in QuickBooks. Duplication is therefore the default failure mode, not an edge case, and it is prevented structurally at both layers:

**The linking rule (QuickBooks side).** A posting task can only execute against a profile that is **explicitly linked** to a QuickBooks customer (`qb_customers.profile_id`, unique when set: one portal client maps to exactly one QB customer, enforced by the schema). `customer.create` **never fires automatically**. An earlier draft of 8C had "create on first sight of a payment"; that is deleted. Creating a QB customer is always an explicit admin choice ("this client is genuinely new to the books"), approval-gated like every posting.

**Two onboarding directions, both ending in a link:**

1. **QB-first (the migration cohort's happy path).** The 8A mirrors give the admin dashboard the full authoritative QuickBooks customer list before any posting exists. The Clients tab gains an "Onboard from QuickBooks" path: pick the QB customer, the create-client form prefills from the mirror (name, email), and the resulting profile is **born linked**. A mirror row that is already linked cannot be onboarded twice.
2. **Portal-first (clients created before 8A, and genuinely new clients).** A linking control on the admin client detail suggests QB matches by normalized identity (email lowercased, phone E.164 via the existing `phone.ts`, case/whitespace-insensitive name compare); the admin confirms with one click, or explicitly chooses "new to QuickBooks" which enqueues the approval-gated `customer.create`.

**At posting time (8C), the queue enforces it.** A payment for an unlinked profile lands in `needs_review` with the match suggestions attached; nothing posts and nothing is created. The admin resolves it once (link or explicit create) and the payment proceeds; every later payment for that client flows hands-free. Ambiguity (two plausible QB matches) always goes to a human, never a guess (handover 23.9), and links are never auto-merged.

**Portal-side duplicate guard (independent of QuickBooks).** **R53 slices 1–2 dropped the global unique on `profiles.email`**, so two sites can share a contact email. Login identity is the member email. Create-client still warns on an exact email/phone hit or a close name match against existing **sites**, **account members**, or unlinked QB mirrors, and offers **Add site to an account** when the email already belongs to a member or a site contact. It warns rather than blocks on a close name, because two legitimate clients can share a name; the admin decides. Do not auto-merge.

**Organization grouping (R53, before invitations).** **Board shipped 2026-09-07** (`/admin-dashboard?tab=grouping`). Staff More info on sign-off and a New client / Add site live hint shipped 2026-09-12. Lanvac Excel has no org id and no email. Seed one pending site per CODE / QB customer. The grouping board on existing profiles (and later on seeded rows) suggests likely groups (exact NAME on 2+ CODEs, civic keywords as a watchlist, same contact email, QB `parent_list_id` when present). The create form flags the same name / email / civic cases while staff type. It does not attach. Accept / reject only. Never auto-attach. Human sign-off writes `portal_settings.org_grouping_reviewed_at` and is a GO LIVE gate (empty-queue sign-off is allowed so the flip is not blocked before the first import). GO LIVE also refuses if any suggestion is still open. Attach expires leftover per-site invites and turns `auto_onboard` off. See [`docs/MULTI_SITE_ACCOUNTS.md`](./docs/MULTI_SITE_ACCOUNTS.md).

#### 9.5.5 Bulk alignment seed + payment reverse-sync (R39)

Two stakeholder requirements (2026-07-07) that make the portal and the books converge instead of drifting:

**A. Seed the portal from QuickBooks (bulk import with staging review).** Rather than onboarding the migration cohort one at a time, the 8A mirrors feed an **"Import from QuickBooks"** bulk flow so the portal is in full alignment before client-facing rollout:

1. Admin selects mirror customers (or "all active monitoring customers"). For each, the system builds a **draft**: name and email from the mirror; monitoring tier, amount, `started_on` (earliest monitoring invoice on that customer, R49), and `next_due_on` **inferred** from mirrored invoice history (last invoice date + interval projects the next due date). Tier inference is a two-signal match (R41): the **billed amount is the primary fingerprint**, because the four tiers bill at four distinct annual amounts ($299.40 / $419.40 / $479.40 / $539.40 net; matched both net and gross of HST), corroborated by line-item names. A clean two-signal hit prefills with confidence; a billed amount matching no current tier price is flagged **legacy rate**, and the draft keeps the amount actually billed with the nearest tier suggested. Nobody's price changes silently at migration: normalizing a grandfathered rate to current pricing is an explicit admin decision, in the review or later. Inference only prefills; nothing commits without the admin walking the staging review and confirming or correcting each row.
2. Committing creates linked `pending` **sites** (today's `profiles`) + manual-rail services with true due dates. **One site per CODE / QB customer. No invitations are sent at import. Do not auto-group.** When a CODE is present, **GET zones** (R54) and store them. Never auto-write zones to Lanvac during import. Re-running the import is idempotent: already-linked mirror rows are skipped, so the seed can never duplicate. **Client-facing mail stays off for the whole import** (9.5.5C): the daily payment-due cron does not email seeded customers either, even when `next_due_on` is inside the reminder window. After commit, the same grouping board (R53 / 9.5.4) reviews the new rows. Human accept attaches; reject dismisses.
3. **The migration then runs in renewal waves off the Billing tab, only after the station layer (R54), multi-site (R53), the CUA test, grouping sign-off, and the 8C client-mail flip.** Because every seeded site's `next_due_on` is real, the collections console covers the entire business from day one, before any client has touched the portal. As each renewal approaches, the admin invites that **account** (one Account admin setup email for a grouped org with no login yet; a house invite for a one-site account). Do not send one activate-and-add-a-card letter per county CODE. The client activates, optionally switches to autopay (R29), and their first portal-era payment posts to QuickBooks through 8C. Clients who never activate still get reminder emails (once mail is live), still show on the collections board, and their recorded payments still post to the books: portal activation is a client-experience upgrade, not a bookkeeping prerequisite.

To support tier/amount inference, the `qb_invoices` mirror carries line-item names and amounts (not just totals).

**Seed enrichment: alarm contacts + devices (R41).** A seeded client is not fully migrated until their alarm contact list and tracked devices exist in the portal, and neither lives in QuickBooks invoices:

- **Caller ID lists come from Lanvac** (the monitoring station holds the authoritative contact lists and passcodes). The Excel export (dealer 10638) is the seed: import people rows only (`E1`/`E2`/…), not `POL` or the `<<<` end marker, and stamp `lanvac_account_code` + `lanvac_city` on the profile (map Excel CITY to the live directory string). After cutover, every list save still emails McKee, emails the customer once client mail is live (R52), and **also writes the people list to Lanvac** (`POST /api/EmergencyContact/fullupdate` with `usePoliceNumbers: true` and `profiles.lanvac_city`). Police/fire/ambulance stay on the station side. If the API call fails, the portal save and emails still succeed; RDP is the fallback. The activation invitation asks the client to **review their alarm contact list on first login**.
- **Device/battery statuses come from the QuickBooks company To Do list as a one-time import.** That list is where they are tracked today (841 freeform notes: customer name + `No smokes` / `zone N smokes YYYY` / `changes own battery`). The bridge adds `ToDoQuery` to its mirror pass (`qb_todos`). The seed turns each note into **device drafts** in the existing portal shape (`label`, `category`, `installed_on`, `lifetime_years`, matched customer) that ride the same staging review as billing drafts. Because the notes are messy, the parser is **AI-assisted and prefill-only**: each draft carries a confidence score and the **original note** (nothing is discarded if the parse is wrong). High-confidence rows still go through staging; low-confidence rows are a human queue so an admin can rename, recategorize, or skip. After commit, **portal `devices` is the system of record**. Staff stop using QB To Dos for new battery/smoke tracking. We do not keep writing devices back to QuickBooks and we do not keep two lists in sync. A full To Do export is not needed in gather; the bridge reads the live list when 8A runs. This enrichment is part of a complete migration, not a gate on the bookkeeper sitting or on 8C payment posting.
- **Per-client migration checklist.** Admin client detail and a Billing-tab filter surface each seeded client's state: imported, `started_on` confirmed (R49), alarm contacts entered, devices entered, invited, activated. Partially migrated clients are always visible; nothing is silently skipped, and "done or not" never depends on memory.

**Migration outreach mechanics (R40).** The renewal wave is semi-automated: automation finds and drafts, a human sends, because the stakeholder intent is to guide each customer individually.

- The daily cron flags seeded, never-invited **accounts** whose earliest site `next_due_on` enters the reminder window; they surface as an **invite queue** on the Billing tab and in the collections digest, so nobody's renewal slips past without an outreach decision. Honor `accounts.auto_onboard` (R53): while off, do not auto-draft a house invite. Multi-site with no activated Account admin uses the **Account admin setup** template (one letter for the whole account), never one invite per site.
- The email for a one-site house is a combined **renewal notice + activation invitation**: amount due, due date, legacy payment instructions (paying is never blocked on activation), the client's personal activation link, and the autopay pitch. Sent per-account (or small waves) with one click from the Billing tab. Prefer **Appoint account admin** before any grouped bill is due so the first letter is not the first time they hear about the portal.
- Visibility: sent from the McKee Security address with reply-to `info@mckeesecurity.ca` (a client's reply lands in the team inbox on the same thread) and **a copy delivered to the admin inbox** on every send, so the team sees exactly what the client received and can follow up verbatim. The client row records invited-at vs. activated-at, so "invited two weeks ago, never activated, renewal in 10 days" is a visible follow-up state, also carried in the collections digest.
- Activation never requires switching rails (a client can activate and stay on e-transfer indefinitely); autopay is strongly encouraged in the copy, never enforced. A full auto-send toggle can be added later once the flow is proven, but it starts human-triggered.

**B. Payment reverse-sync (QuickBooks to portal), payments only, echo-suppressed.** If the bookkeeper records a monitoring payment directly in QuickBooks Desktop (cheque handed over, e-transfer reconciled at the bank), the portal must learn about it, or it will keep dunning a client who already paid. Design:

1. The bridge adds an incremental `ReceivePaymentQuery` (modified since last sync) to its poll cycle and posts results to `/api/qb/payments`.
2. The cloud reconciler applies **echo suppression** first: any TxnID the portal itself created (stamped on a posted `qb_tasks` row) or already imported (`manual_payments.qb_txn_id`, unique when set) is skipped. This is what makes two-way payment flow safe: a portal payment that posts to QB in 8C can never bounce back and advance `next_due_on` twice.
3. A genuinely new QB payment resolves through the link: linked customer + exactly one matching active service + amount consistent with the billing cycle = **auto-apply** through the same domain path as `recordManualPayment` (ledger row marked as QuickBooks-recorded with the TxnID in the note, `next_due_on` advanced, `due_alerted_at` cleared, `unpaid` flipped to `active`, client confirmation email when an email exists and client mail is live). Anything ambiguous (unlinked customer, multiple candidate services, amount mismatch beyond tolerance) parks in `needs_review` on the Accounting tab; never a guess (handover 23.9).
4. Schema deltas (Migration 8C): `manual_payments.qb_txn_id` text, unique when set; `recorded_by` relaxed to nullable strictly for system-recorded rows (CHECK: null only when `qb_txn_id` is set), with `recorded_by_email` carrying `quickbooks-sync`.
5. Freshness bound: QB-keyed payments appear in the portal within one bridge poll cycle while the office PC is on, or when it next comes online. The daily reminder cron tolerates that race: once client mail is live, worst case a reminder goes out the same day the cheque is keyed, and the import clears the guard immediately after.
6. **Same-payment collision guard (R40).** The one real double-entry race: the admin records a cheque in the portal *and* the bookkeeper keys the same cheque into QuickBooks before either sync fires. Echo suppression cannot catch this (two distinct entries, no shared TxnID), so both directions carry a proximity check, and a suspected collision **never auto-applies; it parks**:
   - *Outbound:* before a portal-recorded manual payment posts to QB, the executor checks the payment mirror for a same-customer, same-amount QB payment within a +/- 7-day window that the portal did not originate; a hit parks the task `needs_review` ("was this cheque already keyed into QuickBooks?").
   - *Inbound:* before an imported QB payment auto-applies, the reconciler checks for a same-customer, same-amount portal ledger row in the same window with a pending or posted task; a hit parks it `needs_review` ("possible duplicate of a payment already recorded in the portal").
   - Worst case the same cheque becomes a one-click human resolution on the Accounting tab; it can never double-post to the books or double-advance `next_due_on`.

**Payment origination hierarchy (plain-language summary).** Stripe autopay payments originate in the portal, always; QuickBooks never sees them first, it only receives the posting. Legacy payments (cash / cheque / e-transfer) are *preferably* recorded in the portal dashboard (which posts to QB automatically), but recording them directly in QuickBooks is a supported fallback: the reverse-sync carries them to the portal. Whichever system hears about a legacy payment first tells the other exactly once; if both hear about it independently, the collision guard hands it to a human instead of guessing.

**Direction limits (scope fence).** This is **payments-only** bidirectionality. Customer edits made in QuickBooks update the read-only mirror (visible drift, resolvable on the link screen), never rewrite portal profiles; portal profile edits never touch QuickBooks except through explicit approval-gated tasks. Full two-way record sync is deliberately out of scope: it is where sync systems go to die, and the money trail does not need it.

#### 9.5.5C Client-mail go-live gate (import safety)

Importing real emails and real `next_due_on` values would otherwise start the automated onboarding path: create-client / resend invitation, the daily payment-due reminder, payment receipts, caller-ID client notices, Stripe success mail, and device-expiry client notices. Those must stay silent until production is actually live.

**`portal_settings`** is a singleton (`id = 1`). `client_mail_enabled` defaults **false** and stays false through the station layer (R54), multi-site (R53), the CUA test, 8A/8B, and the bulk seed. Reads fail closed (missing row or query error = no client mail). A pause is not an `email_failure` alert. `due_alerted_at` is stamped only when a reminder actually sends, so the first reminder after the flip is not skipped. `expiry_alerted_at` follows the same rule when the site has an email: do not stamp while the client notice is held (R14). GO LIVE also refuses until `org_grouping_reviewed_at` is set (empty-queue sign-off is allowed) and no grouping suggestion is still open. Sign-off is on the Grouping tab (`/admin-dashboard?tab=grouping`).

**Staff mail stays on:** collections digest, card-failed, device-expiry admin, caller-ID admin, account-change admin.

**The flip is an 8C human step, not an import step.** Admin Billing tab: typed confirm `GO LIVE`. Do not turn it on until all of these are true:

1. The Lanvac station layer (R54) and multi-site accounts (R53) have shipped, and the CUA playbook ([`docs/PORTAL_CUA_TEST.md`](./docs/PORTAL_CUA_TEST.md)) is clean or every fail is accepted.
2. The client import is complete and a sample of accounts is spot-checked.
3. Organization grouping is signed off (`portal_settings.org_grouping_reviewed_at`).
4. The portal is working the way you expect (billing, contacts, devices).
5. The QuickBooks bridge is on the live company file (`McKee Security Live.QBW`), not PORTAL-TEST (`qb_bridges.mode = live` and `expected_company_file` is the live path).
6. You are ready for customers to receive portal email.

Pause is one confirm and needs no phrase. The same banner shows on every admin tab and on client detail while mail is off.

**Out of this gate (still send):** website contact / apply / inquiry forms, Starlink rental reminders (internal), and Supabase Auth mail (password reset for people who already have an auth user). Activation uses `auth.admin.createUser` and does not send a confirmation email. Marketing and Starlink are live products; they are not the import cohort. **Import rule:** do not create Auth users or start Stripe Checkout for real client emails until GO LIVE. Those two paths can still email the address outside `dispatchClientEmail`.

**Admin caller-ID changes while paused (R24):** `client_notified_at` is not stamped. After GO LIVE those saved diffs are not retroactively emailed. Re-save or accept that the dispute-backstop mail starts on the next change.

**Deploy before import.** The hosted row is already false. Old website code does not read the flag. Ship this website change to Vercel before creating or importing real clients, or production will still send the old way.

**Staff digest during import is expected.** The first daily cron after a large seed will email the office a collections digest of everyone due or overdue. That is not customer mail.

#### 9.5.6 VoIP in the accounting rail (R42 deltas)

Phase 9 shipped VoIP **before** Phase 8 precisely so nothing here is monitoring-shaped by accident. What changes and what does not:

- **Nothing structural changes.** The enqueue paths (Stripe webhook, `recordManualPayment`), the task queue, echo suppression, and the collision guard are all keyed on `service_id` / customer / amount, not on service type. A VoIP payment posts through the identical `sales_receipt.create` path.
- **VoIP posting items (D12).** Portal VoIP sales receipts use `VoIP - Residential` or `VoIP - Commercial` (and `VoIP - Number Port Fee` for a port) on **4000 · Product Sales**, class `VOIP`. Create those items in live before the test copy. Do not invent a VoIP Income account.
- **The bulk alignment seed (9.5.5A) stays monitoring-only.** There are four VoIP customers; the stakeholder enters them manually (R42/D14) and links them to their QB customers with the standard 9.5.4 link control. The tier-inference fingerprint is not extended to VoIP amounts: the 3.12 derived totals are not a small closed set, and a handful of manual entries do not justify one.
- **Reverse-sync matching now spans multiple services per client.** A client can hold monitoring and VoIP; a QB payment matches by amount against each candidate service's expected invoice (monitoring: monthly rate x 12; VoIP: the derived monthly total x 1). Distinct amounts and intervals make clean matches the norm; anything ambiguous already parks `needs_review` (9.5.5B rule 3), so no new mechanism is needed, just test coverage in the 8C gate. A VoIP port-fee payment is a one-time amount and must not be matched as a monthly renewal.
- **The MCP revenue tools report per service type.** `get_monthly_revenue_summary` splits monitoring vs VoIP so the answer to "what does VoIP bring in?" is one tool call.

#### 9.5.7 Company file hygiene (R51, D17)

Two programs, two files, one PC. Do not conflate them.

**The local bridge (8A) is the only new program on the QuickBooks PC.** It is a Windows service that opens one company file through the Desktop SDK and polls the cloud outbound. **The MCP server (8D) is not installed on that PC.** It is a Vercel route. Staff ask it questions from Cursor; it reads Supabase mirrors and enqueues tasks. It never speaks qbXML.
...
**ListIDs survive backup/restore.** Portal links created from the copy's customer ListIDs still match the live file at 8C. New customers or payments created *inside the copy during testing* do not exist in live. Those sandbox TxnIDs must never be treated as live-posted (`qb_bridges.mode = sandbox`; switching to `live` does not replay sandbox posts).

**Wrong-file guard.** The bridge compares the open company file to `expected_company_file`. Mismatch: refuse every write. In `live` mode, also refuse to poll. The Accounting tab shows expected vs reported path and `sandbox` / `live`.

### 9.6 VoIP phone service (Phase 9; R42, rate card R50)

VoIP is the monitoring template applied to a second service type, which is exactly what proves the rails are reusable before Track 2 needs them. The 2026-07-18 per-line model is retired (R50).

- **Catalog:** two plans (4.1). `residential` ($34.99/mo base) and `professional` (displayed **Commercial**, $59.99/mo base). Each base includes 1 number + 1 user seat. Additional numbers $4.99 on both plans. Additional seats $24.99, commercial only. Residential has no seat add-on (DB CHECK + `normalizeVoipConfig`). Labels live in `service-labels.ts`; the rate card and derivation live in `billing.ts` (`voipMonthlyCents`, `serviceMonthlyCents`, `withHstCents`). `planMonthlyCents()` returns the **base** only.
- **One system, one figure:** `services.number_count` (1..100), `seat_count` (1..100), `port_count` (0..100). Stored `monthly_amount_cents` is always the **derived total**, so reminder emails, collections, KPIs, and dashboard displays stay unchanged. Phones and handsets add nothing. Precedent: Haliburton Automotive, three handsets, one number, $59.99 total.
- **Client document:** one line naming the service and coverage (`voipInvoiceDescription`). Base and add-ons are never itemised as separate lines. Stripe subscription quantity is always 1. Checkout and `updateVoipConfigAction` swap the single price (`priceForVoipAmount`, `proration_behavior: none`). The webhook writes the stored amount back to the derived total if Stripe's unit amount ever disagrees.
- **Port fee:** $49.99 one-time per number still uncharged (`port_count - port_fee_charged_count`). Charged via `chargeVoipPortFee` as a one-time Stripe invoice (autopay) or a ledger row that does **not** advance `next_due_on` (manual). The count is reserved before Stripe is called, so a double click cannot open two invoices; a failed card charge rolls the count back and voids the draft. Invoice metadata includes `service_id`, `profile_id`, and `kind=voip_port_fee` so `invoice.paid` is stored in history and explicitly skipped as a renewal. Never a subscription line, never inside an installation invoice. Ports cannot exceed numbers.
- **Recurring vs installation:** monthly service bills on its own invoice on its own cycle. It is never folded into a project total, payment threshold, or payment split.
- **Internal cost (admin only):** $5.95 per user seat per month (`VOIP_SEAT_COST_CENTS`). DID cost $0.00 with `VOIP_DID_COST_CONFIRMED = false` until BrightPBX confirms. Never shown on a client document or the client dashboard.
- **Billing interval:** monthly on both rails, enforced by a DB CHECK and by `updateServiceBilling`. An annual VoIP interval would make reminders quote 12x while Stripe still charged the monthly price. Unlike monitoring's annual invoicing. The interval-aware machinery from R26 still handles monitoring.
- **Stripe:** `scripts/stripe-voip-setup.mjs` creates/updates the two plan products (names re-synced to Residential / Commercial) plus the Number Port Fee product, all tagged with a metadata marker. The portal finds or creates those prices at runtime (`resolveVoipCatalogPriceId`). No Vercel price-ID env vars are required; optional `STRIPE_PRICE_VOIP_*` overrides still win if set. The existing HST tax rate applies at checkout unchanged.
- **Admin:** create-client VoIP card collects plan, numbers, seats (commercial only), and ports, and previews the monthly total plus HST and the separate port fee. Client detail ServiceRow edits the same fields (`updateVoipConfigAction`) and can charge the port fee. Plan pickers still show the **base** rate first (`tierOptionLabel`). Cadence note: "One monthly amount for the whole system... never folded into an installation invoice." `admin_create_client` takes `p_voip_tier` / `p_voip_numbers` / `p_voip_seats` / `p_voip_ports`. Cancel / Pause / Restart and plan or numbers/seats changes on a card customer update that customer's Stripe subscription (9.1). A cancelled service must be Restarted before the plan or coverage can change.
- **Client:** a read-only VoIP card (plan, monthly total, coverage, "call McKee to change"); the shared Billing & Payments card carries amount, next payment, autopay setup, and history. No internal cost, no rate-card breakdown.
- **Existing customers:** four as of 2026-08-14. No import tooling: the stakeholder creates them manually via the create-client flow, **deferred by stakeholder choice (2026-07-18) until the Stripe go-live**, and links them to QuickBooks in Phase 8A.
- **Gate:** `scripts/voip-check.mjs` (worked figures, schema round-trip, residential seat lock, RPC args, Stripe catalog + port-fee price) plus the full build. Marketing-site VoIP pages are unchanged; this phase is portal + billing only.

---

## 10. Phase Plan

**Execution order: 0 -> 1 -> 2 -> 3 -> 4 -> 5 -> 7 -> 9 (VoIP, executed 2026-07-18) -> 8 (Track 1: core portal + VoIP + accounting automation, R37/R42), then 6A -> 6B (camera cloud backup, Track 2, post-launch).** Phase numbers are kept stable because the rest of this document references them; 6A/6B run after Track 1, and Phase 9 ran before Phase 8 by stakeholder direction (the accounting rail should be designed against the full service catalog). Phase 8A may start in parallel with the Phase 7 human launch items; the Stripe test-to-live switch is preferably coupled to 8C (9.5.3). Each phase ends with a test gate; do not start the next phase with a failing gate. **[HUMAN]** marks stakeholder checkpoints (handover Section 21 model).
...
Track 1 scope: build the complete billing rails on **both methods** (R22): Stripe checkout/webhooks for autopay, and the manual-payment ledger + reminder machinery for legacy payers. Monitoring prices go live if Q3 confirms in-portal monitoring payment. The cloud backup 9-cell matrix (9.2.7) is planning-only and **not assignable or client-payable until Track 2 ships** (the service cannot run before ingestion exists): admin controls remain visible but disabled under R44, and server actions reject stale or hand-crafted assignment requests. Once launched, all plan changes and cancellations are admin actions per R21.
...
- [ ] After the Phase 6A/6B launch gates and production sign-off, flip `SERVICE_LAUNCH_STATUS.cloud_backup` from `in_development` to `available`; verify the create-client and client-detail selectors unlock, then run the service, billing, RLS, and mobile gates before assigning the first real plan (R44)
...
**Gate:** handover Section 22 checklist fully satisfied; crons observed firing in preview (including a payment-due run that holds the client reminder while mail is paused, or reminds a test manual payer exactly once per cycle when mail is live); sign-off recorded in Progress Log.

### Phase 8: QuickBooks Accounting Automation (Track 1 close-out; R37, architecture in 9.5)

Sub-phases gate independently. 8A and 8B build against the **portal-test copy** of the live company file (R51 / 9.5.7). The live file stays the business books and keeps TSheets. The live file is first opened by the bridge at 8C, after the bookkeeper mapping exists. Cloud work may run in parallel with the Phase 7 human launch items; the Windows install waits on D12 + D17. The Stripe test-to-live switch is preferably coupled to 8C (9.5.3).
...
- [x] `/api/qb/poll`, `/api/qb/report`, `/api/qb/mirror` routes (2026-08-17): per-bridge secret, SHA-256 hash server-side, empty `tasks` until 8B, mirror ingest does not touch `profile_id`. Register/rotate script; `qb-bridge-check.mjs`. No Windows binary.
- [x] Client-mail go-live gate (9.5.5C, 2026-08-22): `portal_settings.client_mail_enabled` defaults false on hosted. Client invitations, payment reminders/receipts, caller-ID notices, Stripe success, and device-expiry client mail no-op until the 8C flip. Staff mail stays on. `due_alerted_at` is not stamped while paused. `expiry_alerted_at` is not stamped while paused if the site has an email (2026-08-22). Billing tab typed `GO LIVE` confirm. Do not flip during import.
- [x] **Lanvac station layer (R54) schema + read UI** (2026-08-22). [`docs/LANVAC_STATION.md`](./docs/LANVAC_STATION.md). Server `lanvac-api.ts` (POST Account / Zone / Historic only). Tables `lanvac_zones`, `lanvac_account_state`, `lanvac_signals`, `lanvac_station_events`. Client SELECT own; no client writes. Write-only zone columns dropped from `lanvac_zones` (JWT SELECT would have returned them after hosted auto-grant). Cache writes in `lanvac-station-store.ts` (not a server action). Admin + client read cards. Actions take `profileId`. CODE change drops the cache.
- [x] **Lanvac station writes / on-test (R54 remainder)** (2026-08-22). `lanvac_zone_write` (admin SELECT only). O5985 gate. Admin zone create/update/delete with reason + staff email. **Account on-test only** (admin and client; client 120s cooldown). No per-zone on-test UI or action. Carbon monoxide type writes refused. Alerts badge while on test. Live O5985 probe: account OnTest/OffTest 200; unused zone 7 BUR create/update/delete; pulled live zones are not PUT. `fullupdate` still off.
- [x] **Multi-site accounts (R53) slices 1–2** (2026-08-28). `accounts` + `account_members`, client `account_id`, membership RLS, insert trigger, unique email dropped. `resolvePortalSession` / `requireSelectedSite`. Orphan, OAuth, cleanup, and password treat `account_members.user_id` as a linked login. GO LIVE refuses without `org_grouping_reviewed_at`. Audit the same day closed Stripe email reuse, delete-Auth wipe, and per-site disable. Switcher / People / last-owner revoke / grouping board are later slices. Locked model: [`docs/MULTI_SITE_ACCOUNTS.md`](./docs/MULTI_SITE_ACCOUNTS.md). Do not start the Windows bridge or real import in this item.
- [x] **Multi-site accounts (R53) slice 3** (2026-08-29). Last Account admin cannot be revoked (`revokeAccountMemberAction` + staff People with access). Delete confirm copy names **this site**; multi-site copy says the other sites and the login stay. Do not start the Windows bridge or real import in this item.
- [x] **Clients list linked-account chip** (2026-08-30). Account column: name + site count when an account has two or more sites. Linked accounts / One site filter. Search by account name. Single-site rows show a dash. List service chips are abbreviated. Staff and client portal shells are `max-w-[90rem]`.
- [x] **Multi-site accounts (R53) slice 4 first increment** (2026-08-30). Two Clients-tab buttons (New client / Add site to an account). Email collision offers Add site. Account card: name, site links, auto-onboard toggle. Create and resend honor `auto_onboard`. No invitation on Add site. Do not start the Windows bridge or real import in this item.
- [x] **Staff help tips + create-form grouping hint + client header identity** (2026-09-12). Reusable More info modal on grouping sign-off, automatic onboarding, and New client / Add site. Plain-language why / what / not. Copy written for live mail. Draft same-name / same-email / civic hint while staff type. First/last is the site label (Dysart / Library). Client header shows site name · CODE (Account line when useful). Switcher is still slice 5.
- [ ] **Multi-site accounts (R53) remaining slices.** Still open: Appoint account admin + attach/move, client switcher / sites list / People, emails. Grouping board shipped 2026-09-07. Header identity shipped 2026-09-12.
- [ ] **CUA playbook** ([`docs/PORTAL_CUA_TEST.md`](./docs/PORTAL_CUA_TEST.md)). Living doc: update it in the same commit as each UI slice. After deploy, a computer-using agent runs it. **Last gate before `qb-bridge/` or real import.**
- [ ] `qb-bridge/` v0 (C#/.NET Windows service): QB session management via Desktop SDK/qbXML (not Web Connector), CustomerQuery/InvoiceQuery/ReceivePaymentQuery against the **portal-test copy**, refuse to run if the open file ≠ `expected_company_file`, mirror push on an interval, service autostart + crash recovery, structured local logging. **Do not start until R54, R53, and CUA are done.**
...
- [ ] Identity linking (9.5.4): unique `qb_customers.profile_id` link, client-detail link control with normalized match suggestions (email/phone/name), unlink with confirm; **"Onboard from QuickBooks"** path on the Clients tab creating profiles born linked from mirror rows (already-linked rows excluded)
- [ ] Create-client near-duplicate warning (9.5.4 / R53): exact email/phone or close-name match against existing sites, account members, and unlinked mirrors. Email collision offers **Add site to an account**. Warn, not block, on a close name. Never auto-merge
- [ ] Bulk alignment seed (9.5.5A): "Import from QuickBooks" staging review — drafts prefilled from mirrors (name, email) and invoice history (tier from the billed-amount fingerprint corroborated by line-item names per R41, legacy rates flagged and preserved as billed, `started_on` from the earliest monitoring invoice per R49/D15, projected `next_due_on`), admin confirms/corrects each row, commit creates linked `pending` **sites** + manual-rail services, **one site per QB customer, no invitations, no auto-group**; idempotent re-runs (linked rows skipped). Grouping board (R53) reviews the new rows after commit. `qb_invoices` mirror carries line items for the inference
- [ ] **[HUMAN]** D15: when entering or reviewing a monitoring client (create-client, add-service, or seed staging), confirm `started_on` is the day they first started monitoring, not the invitation or activation date. Spot-check the seed's inferred first-invoice dates against known long-standing accounts
- [ ] **[HUMAN]** D16: gather cost per monitored client for each current monitoring tier (monitoring-station / communicator cost, not retail). Required before any monitoring profitability view; not required to start 8A or to post payments in 8C. Store later in a dated rate table in the Starlink `unit_costs` pattern — do not invent numbers to unblock posting
- [ ] Seed enrichment (R41): bridge `ToDoQuery` mirror of the QB company To Do list (`qb_todos`); notes parsed into device drafts (label, category, `installed_on`, `lifetime_years`, matched customer, confidence, original note) in the same staging review; low-confidence drafts are a human queue; after commit, portal `devices` is the system of record and QB To Dos are not kept in sync; Lanvac caller ID import per D13 (one-time importer if the export is machine-readable, manual entry otherwise; admin-attributed, no client notifications during seeding); per-client migration checklist (imported / `started_on` confirmed / alarm contacts entered / devices entered / invited / activated) on client detail + Billing-tab filter
- [x] **[HUMAN]** D13 API contract ingested and write-tested 2026-08-16 on `O5985`. Portal station fields shipped (`lanvac_account_code`, `lanvac_city`). Dropdown is Ontario directory cities only, McKee-frequency first. Vercel env set; thank-you email sent. Remaining: match Excel CODEs to portal clients at seed time (map CITY aliases); wire `fullupdate` on list save (not yet). Device samples are in `ACCOUNTING_GATHER_DATA.md`.
- [ ] Migration outreach (R40): cron-driven invite queue on the Billing tab (seeded, never-invited, renewal inside reminder window); one-click combined renewal + activation email (amount due, legacy payment instructions, activation link, autopay pitch, prompt to review the alarm contact list on first login); reply-to `info@mckeesecurity.ca` + admin copy on every send; invited-at vs. activated-at visible on client rows and in the collections digest
- [ ] **[HUMAN]** stakeholder runs the seed against the real mirror list and spot-checks a sample of drafts (tiers, amounts, `started_on`, due dates, devices, contacts) before commit

**Gate 8A:** mirrors match the portal-test copy (customer count, a spot-checked invoice, a to-do); opening the live file while the bridge is in sandbox mode is a refused mismatch, not a silent switch; the bridge survives a reboot and a QuickBooks-closed state gracefully; the portal is fully functional with the bridge offline; bridge secret rotation works; onboarding from a mirror row creates a pre-linked profile and the same row cannot be onboarded twice; a duplicate create attempt (same email, same phone, near-identical name) triggers the warning; the bulk seed round-trips correctly (seeded count = selected count, re-run creates zero new rows, every seeded client lands on the Billing tab with a real due date and a `started_on` inferred from the earliest monitoring invoice); tier inference resolves a known-price customer correctly and flags an off-price amount as legacy rate without changing it; a real QB to-do entry becomes a correct device draft; imported alarm contacts render with passcodes and send no client emails; the migration checklist reflects each seeded client's true state; TSheets still syncs after every copy-open session once live is reopened.
...
- [ ] **[HUMAN]** D12 (mapping): locked 2026-08-16 (sales receipt, 4000 + class, card to `*Stripe` / CIBC / 5800, e-transfer and cheque to 1499 UF then Make Deposit, HST 13%). Remaining: first live card sales receipt inspected. Then flip `qb_bridges.mode` to `live`, set `expected_company_file` to the live path, and reopen the live file. Sandbox postings in the copy are not replayed onto live
- [ ] Stripe rail enqueue: the `invoice.paid` webhook handler also enqueues `sales_receipt.create` against the **linked** QB customer (9.5.4) with item + class + deposit to `*Stripe`; unlinked profiles land the task in `needs_review` with match suggestions attached, and nothing posts or creates
- [ ] Manual rail enqueue: `recordManualPayment` (7.3) enqueues the equivalent `sales_receipt.create` (payment method `Electronic Funds Transfer` / `Cheque` / `Cash`, Deposit To 1499), so those payments post too (same link requirement)
- [ ] `customer.create` is an explicit admin action only ("new to QuickBooks" after the no-match confirmation, 9.5.4), approval-gated like all posting; no code path creates a QB customer implicitly
- [ ] Backfill: one-shot script replays historical `billing_events` (`invoice.paid`) and `manual_payments` through the same idempotent enqueue path (unlinked clients surface as `needs_review`, so the backfill doubles as the link-completeness sweep for the migration cohort)
...
- [ ] Payment reverse-sync (9.5.5B): bridge incremental `SalesReceiptQuery` + `ReceivePaymentQuery` -> `/api/qb/payments`; reconciler with echo suppression (portal-originated TxnIDs skipped via `qb_tasks.qb_txn_id`; imports unique on `manual_payments.qb_txn_id`); clean matches auto-apply through the `recordManualPayment` domain path; ambiguity parks `needs_review`. Migration 8C schema deltas: `manual_payments.qb_txn_id` unique-when-set, `recorded_by` nullable only for system rows
- [ ] Same-payment collision guard (R40): outbound tasks and inbound imports both proximity-check the other system (same customer, same amount, +/- 7 days) and park `needs_review` on a hit; suspected duplicates never auto-post or auto-apply
...
- [ ] **[HUMAN] Client-mail go-live (9.5.5C). Last flip, not an import step.** On the Billing tab, type `GO LIVE` only after: (1) the station layer (R54), multi-site (R53), and the CUA test are done, (2) the client import is complete and spot-checked, (3) organization grouping is signed off, (4) the portal is working as expected, (5) the QuickBooks bridge is on `McKee Security Live.QBW` (`mode = live`), and (6) you are ready for customers to receive invitations, payment reminders, receipts, and notices. Until then the toggle stays off. Pause anytime with one confirm.

**Gate 8C:** client mail stays off until the human `GO LIVE` flip (R52); a Stripe payment and a recorded manual payment each land in QuickBooks correctly with the office PC initially off; a replayed webhook produces exactly one QB posting; posted amounts reconcile to the bookkeeper's mapping (gross, fees, tax); a payment for an unlinked profile lands `needs_review` with suggestions and posts after one-click linking, with **no duplicate QB customer created for an existing one**; a payment keyed directly into QuickBooks Desktop appears in the portal ledger exactly once, advances `next_due_on`, and stops the reminder cycle, while a portal-originated payment never echoes back as a duplicate (verified by running the sync twice); the same cheque entered in both systems parks `needs_review` in both directions instead of double-posting; a QB payment for a client holding **both monitoring and VoIP** auto-applies to the right service by amount, and an ambiguous amount parks `needs_review` (9.5.6); a VoIP payment on each rail posts with the VoIP mapping, not the monitoring one; deleting a client leaves queue history intact.
...
### Phase 9: VoIP Phone Service (Track 1 addition; R42, design in 9.6). Executed 2026-07-18, before Phase 8

- [x] Migrations `20260718190000` (add `'voip'` to `service_type`; enum values cannot be referenced in the transaction that adds them, hence the split) and `20260718190100` (`services.line_count` 1..100, tier CHECK gains `voip in ('residential','professional')`, `admin_create_client` extended with defaulted `p_voip_tier`/`p_voip_lines` so older deploys keep resolving). Applied to production via the Management API fallback and recorded in migration history; `database.types.ts` updated
- [x] Pricing + labels: `VOIP_MONTHLY_CENTS` + shared `planMonthlyCents()` in `billing.ts`; `voip` tiers/labels + `isPerLineService()` in `service-labels.ts`; `voip` label in the email templates
- [x] Stripe (test mode): `scripts/stripe-voip-setup.mjs` created the two products + monthly CAD prices (idempotent, metadata-marked; re-run against live keys at go-live); `STRIPE_PRICE_VOIP_RESIDENTIAL`/`_PROFESSIONAL` added to `PRICE_ENV_KEYS` and `.env.local`
- [x] Rails: checkout sends `quantity = line_count`; webhook `subscription.updated` syncs line count + amount for per-line plans; `updateServiceBilling` amount re-sync generalized from monitoring-only to `planMonthlyCents` x lines; tier changes re-sync amount and preserve lines on per-line plans
- [x] Admin: create-client form VoIP select + lines input; `assignServiceAction` accepts `lineCount`; new `updateServiceLineCountAction` (updates the Stripe subscription quantity when live); client detail ServiceRow shows the lines editor + "for N lines" on the autopay summary; Clients tab service/tier filters + chips include VoIP
- [x] Client: read-only VoIP card (plan, rate, lines, status); Billing & Payments card, reminders, collections, and payment history all work unchanged because the stored amount is the per-line total
- [x] Gate: `scripts/voip-check.mjs` 11/11 against production (RPC with VoIP args, per-line storage and math, tier + line-count CHECK rejections, cleanup, both Stripe prices verified as $34.99/$59.99 monthly CAD); `npm run build` clean
- [x] **[HUMAN]** Add `STRIPE_PRICE_VOIP_RESIDENTIAL` and `STRIPE_PRICE_VOIP_PROFESSIONAL` to Vercel (stakeholder, 2026-07-18)
- [x] Stakeholder UX round (2026-07-18): every VoIP plan made per line (`isPerLineService` keys on service type; residential can carry e.g. a fax line), `professional` relabeled **"Business"** (Stripe product renamed to "McKee Security VoIP Business" via the re-run setup script, which now re-syncs names), create-client form rebuilt into sections (Client Details / Services as one bordered card per service type with the lines input living inside the VoIP card, grayed out until a plan is chosen / Billing), and "How will they pay?" made always visible instead of appearing only after a service is picked
- [x] Rate card R50 (2026-08-13): migration `20260813161031_voip_numbers_seats.sql` replaces `line_count` with `number_count` / `seat_count` / `port_count`, locks residential seats at 1, and extends `admin_create_client` with `p_voip_numbers` / `p_voip_seats` / `p_voip_ports`. `billing.ts` encodes the 3.12 formula and worked figures. Stripe quantity is always 1; `priceForVoipAmount` finds or creates the single monthly price at the derived total. Port fee is a separate one-time charge (`chargeVoipPortFee`). `professional` displayed **Commercial**. Internal seat cost admin-only; DID cost $0.00 unconfirmed
- [x] Rate-card audit (2026-08-13): migration `20260813163800_voip_hardening.sql` adds `port_fee_charged_count`, `port_count <= number_count`, and VoIP-must-be-monthly. Port-fee charges reserve the count first; failed card charges roll back. Webhook skips `kind=voip_port_fee` renewals and keeps `monthly_amount_cents` on the rate card even if Stripe drifts
- [x] Stripe status-sync audit (2026-08-13): webhook honors `cancel_at_period_end` and `pause_collection` so Cancel/Pause cannot be flipped back to Active; stale Checkout after Cancel/Pause cancels the new subscription; plan and VoIP-config edits refused while cancelled; Cancel confirm copy states they stay paid through the period; card-rail port fee with Stripe unconfigured rolls back instead of falling through to the ledger
- [x] Admin + client surfaces updated: create-client and ServiceRow collect numbers / seats / ports and preview the one monthly figure; client dashboard shows coverage, not lines
- [ ] **[HUMAN]** Enter the two existing VoIP customers manually: residential and commercial, each with their real number count, seat count, port count, and next payment date; link both to QuickBooks in Phase 8A. **Deferred by stakeholder (2026-07-18) to the Stripe go-live**
- [x] VoIP catalog prices resolve by Stripe metadata marker (2026-08-13). `STRIPE_PRICE_VOIP_*` env vars are optional overrides, not required. The Number Port Fee no longer needs a Vercel paste.
- [ ] **[HUMAN]** At Stripe go-live: swap `STRIPE_SECRET_KEY` to live and re-run `stripe-voip-setup.mjs` with the live key (or let the first live checkout create the marked products). Monitoring live prices still go in Vercel (Phase 8C coupling, 9.5.3)

**Gate:** scripted checks green against the 3.12 worked figures; a VoIP client on autopay checks out with quantity 1 and the derived total; a numbers/seats change swaps the single Stripe price and the next invoice; a port fee is charged once and does not move `next_due_on`; a VoIP manual payer gets reminders quoting the one monthly figure. The remaining [HUMAN] items do not block Phase 8A.
...
| VoIP takes automatic payments on the same rails as monitoring (stakeholder 2026-07-18, R42; rate card R50) | Phase 9: `voip` service type + 3.12 derived monthly total (one line, quantity 1), shared reminder/collections machinery (4.2, 9.6); posts to the books via Phase 8 like every service (9.5.6) |
...
| Monitoring history and later P&L use the day they were first monitored, not portal invite/activation (stakeholder 2026-08-13, R49) | `services.started_on` required on create-client / add-service for monitoring; seed infers earliest QB monitoring invoice (D15); cost per monitored client by tier gathered before any profit view (D16); invitation and activation dates stay portal-access only |
...
| `PORTAL_ETRANSFER_EMAIL` | unused | 5 — not set. D11 resolved to `dennis@mckeesecurity.ca` in `PAYMENT_INSTRUCTIONS`; no env var needed |
...
| `STRIPE_PRICE_VOIP_RESIDENTIAL/_PROFESSIONAL/_NUMBER_PORT` | optional | 9 — optional overrides. The portal finds or creates these by Stripe metadata marker. Already-set Vercel values still win if present |
...
| `LANVAC_API_BASE` | Vercel only | D13 — **set 2026-08-16** (`https://lanvac.mobi:8843`). Server-only. Adding these does **not** turn on writes. |
...
| Date | Milestone |
|------|-----------|
| 2026-09-12 | **Staff More info help, create-form grouping hint, client header identity.** Click More info for sign-off, automatic onboarding, and Possible linked account. Copy is plain-language why / what / not, written for live mail. New client / Add site flags same name, same email, or a civic-looking name without attaching. First/last is the site label (Dysart / Library). Client header shows site name · CODE, plus Account when the name differs or there are 2+ sites. Switcher is still slice 5. Next is Appoint account admin and Account-card attach/move. Do not type `GO LIVE`. |
| 2026-09-07 | **Grouping board.** Staff Grouping tab reviews same-name / same-email / QuickBooks-job suggestions. Accept links sites and expires leftover invites. Reject dismisses. Empty-queue sign-off is a GO LIVE gate. Hosted McKee House + Bunkie already share an account, so the queue is empty. Refresh runs on the Grouping tab only. Next is Appoint account admin and Account-card attach/move. Do not type `GO LIVE`. |
| 2026-09-05 | **Slice 4 UI polish signed off. Station actor decision locked.** Clients list is two red buttons until a form is open. Account card puts this site first. Alerts on-test matches the Security clock. House writes stay read-only (O5985 gate). Portal already stores who clicked on-test in `lanvac_station_events`. Showing that on Historic is R54b. Per-staff Lanvac passwords are R54c, later. Next implementation is grouping / Appoint / attach. Do not type `GO LIVE`. |
| 2026-08-30 | **Clients list linked-account chip.** Staff Clients tab has an Account column. Two or more sites on one account show `McKee · 2 sites`. Single-site rows show a dash. Filter Linked accounts vs One site. Search matches the account name. Next is slices 4–6. Do not delete the McKee sites. Do not start the Windows QuickBooks bridge or a real import. Do not type `GO LIVE`. |
| 2026-08-29 | **R53 slice 3.** Last Account admin cannot be revoked. Staff delete confirm copy names this site and, when the account has more than one, says the other sites and the login stay. Next is slices 4–6. Do not delete the McKee sites. Do not start the Windows QuickBooks bridge or a real import. Do not type `GO LIVE`. |
...
| 2026-08-23 | **Staff zone form is number, type, and description.** Delay, extra notify, signal/restore codes, and the reason box are hidden. Writes always send McKee defaults, so Edit is back for pulled fire / burglar / low-temp. Carbon monoxide stays delete-only. Add only offers unused numbers. Next implementation is still R53. Do not type `GO LIVE`. |
| 2026-08-23 | **Admin client detail tabs + shared service chrome. Does not change R53 / GO LIVE.** Client on a staff URL gets the R46 wrong-door (not a 404). Client detail is Account / Billing / Security / Devices. Security and Devices use the same red `PortalCard` header as the client Security tab. Billing rows use the shared product icon and color (monitoring red, VoIP teal, cloud sky). Station extras load only on Security. Staff mail deep-links to the matching tab. Zone edit stays off for pulled live zones; the UI says that in plain language. Next implementation is still R53. Do not type `GO LIVE`. |
| 2026-08-23 | **Historic signals UX + Security header.** Grouped, color-coded log with 12-hour dates, day boxes, filters, and paged load-more. On-test chrome is amber; duration presets use Security red. Load-more observes the log scroller (not the window) and appends are de-duped. Next implementation is still R53. Do not type `GO LIVE`. |
| 2026-08-23 | **Security tab + station readout polish.** Security header is page chrome (not a card), with an unpaid CTA. Zones & Signals drops panel / last-signal chips. On-test / off-test status sits above Refresh; duration presets are 30 min–24 hours plus custom days/hours. Start/End follow account state. Zone On test stays the Lanvac zone flag (does not flip on account test). Next implementation is still R53. Do not type `GO LIVE`. |
| 2026-08-23 | **Client portal layout: billing-first Dashboard + Security tab.** Dashboard leads with Billing & Payments, then compact service cards. Zones, Historic, the alarm contact list, and equipment move to a Security tab that appears only with monitoring or leftover station/contact/equipment rows. Existing contacts can be edited (name, phone, passcode) before Save List. Lanvac Account/Zone/Historic pulls and the full caller-ID list load only on that tab. Next implementation is still R53. Do not type `GO LIVE`. |
| 2026-08-22 | **R54 closed.** Account-only on-test, OffTest retry, Historic emails redacted, chip is green only after a restore. MULTI_SITE confirmed per-site. Next implementation is R53 slices 1–2. Do not type `GO LIVE`. |
| 2026-08-22 | **On-test is the whole account only.** McKee does not put a single zone on test. Removed per-zone on-test UI and actions. Portal and R55 stay on `Account/OnTest` / `OffTest`. `on_test_until` is the chip SoR. Live account probe: OnTest 200, zone GET stays off, immediate OffTest can 500 (retry 200). Next is R53. Do not type `GO LIVE`. |
| 2026-08-22 | **R54 writes live-tested on O5985 and audited.** Account OnTest/OffTest, unused zone 7 BUR create/update/delete, then restore. An unused Zone/OnTest probe is not a product path. Account GET has no on-test field; OffTest 500 if already off; 20s write timeout; do not PUT pulled live zones. `-X0070` is Mobi file viewed (ops). Next is R53. Do not type `GO LIVE`. |
| 2026-08-22 | **R54 writes / on-test shipped, O5985 only.** `lanvac_zone_write` is admin-readable, not client-readable. Carbon monoxide type writes refused. Next is R53 after a light multi-site revise. Do not type `GO LIVE`. |
| 2026-08-22 | **R54 read sitting audited and hardened.** Cache-clear is `server-only` (not a callable action). Zone write columns dropped from `lanvac_zones` after hosted auto-grant undid a column revoke. Zone upsert is batched. Pull errors stay generic. Auto-refresh does not stack. Docs say writes next, then R53. Do not start R53. Do not type `GO LIVE`. |
| 2026-08-22 | **R54 schema + read-only UI shipped.** Server Lanvac client, four `lanvac_*` tables (client SELECT own, events admin-only, no client writes), admin Monitoring station card + client Dashboard zones/Historic. Actions take `profileId`. CODE change drops the cache. Writes and on-test stay off. Do not start R53. Do not type `GO LIVE`. |
| 2026-08-22 | **R54 docs + O5985 reads locked. Schema/read UI is the next code sitting.** [`docs/LANVAC_STATION.md`](./docs/LANVAC_STATION.md): seven-zone restore table, Historic color rules, GET type labels vs write enum. Account / Zone / Historic all 200. Writes stay `O5985` until you say go. Sequence everywhere is R54, R53, CUA, then bridge / import. Do not type `GO LIVE`. |
| 2026-08-22 | **R54 station layer documented. Next work is Lanvac zones / Historic / panel / on-test before multi-site.** Locked model: [`docs/LANVAC_STATION.md`](./docs/LANVAC_STATION.md). R55 is the future technician app + Alarmnet. Build order is now R54, R53, CUA, then bridge / import. `O5985` read snapshot is the first sitting. Station writes stay off. Do not type `GO LIVE`. |
| 2026-08-22 | **Pre-implementation audit: R53 written as planned, docs aligned, device-expiry stamp hardened.** `PORTAL_PLAN.md` R53 + 9.5.4/9.5.5/9.5.5C now match [`docs/MULTI_SITE_ACCOUNTS.md`](./docs/MULTI_SITE_ACCOUNTS.md): multi-site then CUA, then Windows bridge / real import; grouping sign-off is a GO LIVE gate. `expiry_alerted_at` is no longer stamped when only the admin alert succeeded while client mail is paused. Companion docs (`ACCOUNTING_PLAN.md`, `ACCOUNTING_GATHER.md`, `AGENTS.md`, handover header, Billing-tab checklist) say the same sequence. Do not start slices 1–2 until asked. Do not type `GO LIVE`. |
| 2026-08-22 | **Client-mail go-live gate shipped and audited (9.5.5C / R52).** Hosted `portal_settings` singleton (`20260822135506`), `client_mail_enabled = false`. All six client portal templates go through `dispatchClientEmail` (invites, payment reminder/receipt, caller-ID notice, Stripe success, device-expiry client). Staff mail unchanged. `due_alerted_at` stays empty while paused. Billing tab typed `GO LIVE`; banner on every admin surface. Docs caught up: Section 8, 6.2, 9.4, 4.2/4.3, handover 7.2/8.1, `ACCOUNTING_PLAN.md`, `ACCOUNTING_GATHER.md`, root/website READMEs, `AGENTS.md`. Flip is an 8C human step after import + live QB file. Deploy this website change before importing real clients. |
...
| 2026-08-15 | **Device migration is a one-time seed, then the portal owns the list (D13 / 9.5.5A).** QB company To Dos (841 freeform notes) are the source for device drafts only. After commit, portal `devices` is the system of record; we do not two-way sync To Dos and staff stop using that QB list for new battery/smoke tracking. Parser is AI-assisted into the existing device shape with a confidence score and the original note kept; low-confidence rows are a human queue. Not a bookkeeper or 8C posting blocker. Full To Do export waits for the 8A bridge. |
| 2026-08-13 | **Caller ID call order.** Contacts store `sort_order` (#1 is first). The list shows #1, #2, #3 and a drag handle (touch + mouse + arrow keys) to reorder. Reorder-only saves write history and email McKee with blue “was #n” rows so Lanvac can be updated. The same phone may appear more than once. Client saves wait 2 minutes between real changes (admins are not limited) so the inbox is not flooded; the wait copy is a pause, not an error. |
...
| 2026-08-13 | **One card starts every approved service.** First-time clients see one “Set up your card to start your services” banner (monitoring, VoIP, outstanding port fee listed). Checkout saves the card on the Stripe customer; the webhook then starts the other subscriptions and charges uncharged ports on that card. Later extra ports are a one-tap charge on the saved card. Update Card keeps automation going. Leftovers (if the follow-through misses) are one “Finish starting” confirm, not a second Checkout. |
| 2026-08-13 | **Hold billing kept; VoIP Stripe prices resolve themselves.** Pause stays because it is not Cancel: Stripe keeps the subscription and stops charging, so Restart does not make the client enter a card again. Cancel ends the subscription at period end. Hold is excluded from collections, booked-revenue KPIs, and port-fee charges, and clears any pending period-end cancel. VoIP catalog prices (Residential, Commercial, Number Port Fee) are found or created by Stripe metadata marker. No Vercel paste for those IDs; optional `STRIPE_PRICE_VOIP_*` overrides still win. |
| 2026-08-13 | **Stripe cancel/change sync audit.** Webhook status mapping now honors `cancel_at_period_end` and `pause_collection` (Stripe still reports `active` for both, which was flipping Cancel/Pause back to Active). Stale Checkout after Cancel/Pause cancels the new subscription instead of reviving the service. Plan and VoIP-config edits are refused while cancelled. Cancel confirm copy states they stay paid through the period. Card-rail port fee with Stripe unconfigured rolls back instead of writing a ledger row. Section 9.1 documents `proration_behavior: none` (was stale "proration default"). |
| 2026-08-13 | **VoIP rate-card audit hardening.** Port fee can be charged only for ports not yet invoiced (`port_fee_charged_count`); the Charge button disappears after a successful charge. VoIP is locked to monthly at the database and in billing save. Ports cannot exceed numbers. Port-fee invoices stamp `profile_id` and are ignored by the renewal webhook. Stripe subscription description is updated when numbers or seats change. A failed card charge no longer reports success. |
| 2026-08-13 | **VoIP rate card (R50 / company knowledge 3.12).** Retired per-line billing. One subscription, one line item, quantity 1, charged once per system. `number_count` / `seat_count` / `port_count` replace `line_count`. Commercial base $59.99, residential $34.99 (each includes 1 number + 1 seat); additional numbers $4.99; additional seats $24.99 commercial only; port fee $49.99 one-time. HST applied to the sum once. Recurring never folds into installation. Stripe catalog products renamed/described to match; Number Port Fee product added. `professional` displayed Commercial. Internal seat cost $5.95, DID $0.00 unconfirmed, admin-only. Two existing customers still entered by hand at Stripe go-live (D14). |
...
| 2026-08-13 | **Starlink Profit tab.** Per-kit and fleet P&L (week / month / all-time) from rental `amount_received` minus each kit's Starlink subscription. Deposits are excluded. Monthly rates live in `unit_costs` with history (`effective_from`) so a plan change does not rewrite past months; seeded from the live plans as of today (Starlink 1 and 3 Roam Unlimited $200, Starlink 2 Roam 300GB $110). Saving a rate from the tab inserts a new row, or updates the same day's row. See `website/src/lib/starlink/profit.ts` |
| 2026-08-13 | **Portal identity and service-color overhaul (R45/R46).** Client vs staff sign-in no longer share the same chrome: client screens (sign-in, activation, password setup, reset) use the shared `AuthFrame` with the McKee shield and brand-red card; the admin door is an amber staff console with a "Staff only" mark and a prominent link back to Manage Account. An admin who lands on `/user-dashboard` gets a wrong-door page and a link to `/admin-dashboard` instead of unused-service cards. Owned services stay primary; unused monitoring/VoIP/cloud sit in a dashed "Other McKee services" band below billing. Shared `SERVICE_THEME` colors (monitoring red, VoIP teal, cloud sky) apply to client cards, admin chips, service rows, and overview pills. Admin Overview is grouped into Clients / Billing / Needs attention with icons and green/amber/red tones; Alerts tab always shows a 0-green or N-red badge. Caller ID and devices hide unless the client has current monitoring or historic rows; create-client dynamically opens optional contact and device seeders when a monitoring plan is chosen. Invitation email heading is now "Your portal is ready". Fresh monitoring accounts with an empty contact list get a top banner that scrolls to `#alarm-contact-list` |
| 2026-08-05 | **Starlink admin mobile/desktop hardening audit.** Root cause of the worst finding: the booking modal's card carried `overflow-hidden` (for the rounded gradient bar), which makes it a scroll container, and a `sticky` child sticks to its nearest scrolling ancestor — so the sticky header and footer had never worked. On a 390px phone the panel is ~2,090px tall, meaning Save was three screens down on every booking. One-word fix to `overflow-clip` (clips identically, creates no scroll container); measured header pinned at 12px and the footer at 652px of a 664px viewport after a 600px scroll, and again in 740x360 landscape. Also: every field went `text-base sm:text-sm` because iOS Safari zooms the viewport whenever a focused input is under 16px (once per field, on a form that is mostly fields); the custom combobox now claims Escape via `defaultPrevented` instead of `stopPropagation` (React 19 delegates to the same node as the modal's own listener, so the first Escape was discarding the whole form), keeps focus on its trigger through pointer selection, dismisses on `pointerdown` (iOS synthesises no `mousedown` for non-interactive targets), scrolls only for keyboard moves, and regained the type-ahead the native `<select>` used to provide free; scroll-lock split into a mount-scoped effect that restores the previous value rather than `""`; kit availability gained its own glyph because the option's colour dot is the kit's *calendar* colour, so a red kit read as "booked" while free; a clash on a returned/cancelled booking is now a neutral note rather than a red alert and a confirm dialog, since the DB exclusion constraint only binds confirmed/active rows; the calendar grid moved to `lg` (between 640–1000px seven columns left ~16–34px per chip); the mobile month arrows now re-point the agenda instead of only changing the heading; `Section`/`Field`/`inputClass` extracted to `starlink-admin/form-ui.tsx` so the fleet tab matches the modal; one status-icon map for the whole admin (badges and indicators disagreed on the same card); toast un-clamped from half the viewport (`left-1/2` with auto width shrink-to-fits to viewport-minus-offset); call FAB suppressed on `/starlink-admin` as Data Drops already does. Emails: all `color:` values moved off `rgba()` (Outlook's Word engine drops the declaration outright, leaving near-black text on a near-black card), the field value wrapper became a `div` (the digest's block items were orphaning the `p`), and `word-break` added so one long address cannot make Gmail iOS shrink the whole message. New reusable gate `scripts/starlink-admin-ui-check.mjs`: 26/26 at 390/740x360/768/1024/1920, exercising overflow, tap sizes, focused font size, the sticky bars, and the full availability path incl. the confirm dialog naming the conflicting booking |
| 2026-07-18 | **Email identity completed across portal + website forms.** Confirmed the main HTML shell already uses the real McKee favicon shield at 48px; kept that single same-domain HTTPS image rather than attempting an image-heavy email. Since email subjects are text-only, `sendEmail()` now guarantees a consistent `McKee Security \| ...` prefix for every current and future message. Removed the remaining emoji from form subjects, portal metadata, and plaintext fallbacks; cleaned duplicate company wording from invitation/contact-list subjects; added a concise hidden preheader from each template's inbox label. This preserves recognizable branding even when images are blocked while avoiding promotional emoji and protecting deliverability. Build clean; rendered-template assertions confirmed shield present, preheader present, CTA arrow absent, branded plaintext, idempotent subject prefix, and no subject emoji |
| 2026-07-18 | **Pre-launch Camera Cloud Backup admin template locked (R44).** The create-client Services fieldset keeps its dedicated cloud card, now grayed with an **In Development** badge, visible planned 7/30/90-day retention options, and a disabled selector. Every client detail without cloud service gets the same separate disabled template beneath active services; the general Add Service picker and Clients filters exclude/disable cloud while unavailable. Shared `SERVICE_LAUNCH_STATUS.cloud_backup = 'in_development'` is the single unlock point. Both `createClientAction` and `assignServiceAction` independently reject cloud assignment, protecting against stale pages and crafted requests. Phase 5 language corrected (no manually arranged cloud rows pre-launch), and Phase 6A now owns the explicit post-gate unlock checklist. Build clean; updated services gate **16/16**; mobile audit passed all nine portal views with zero horizontal overflow |
| 2026-07-18 | **Email polish + consent-based Camera Cloud Backup interest list (R43).** Global branded email shell now uses the real McKee shield mark rather than platform emoji and no longer appends a right arrow to green CTA buttons; invitation plaintext icon reduced to a check mark. Client dashboard ordering fixed: assigned services and Billing & Payments remain first; missing-monitoring promotion moved to the bottom; missing cloud backup now gets the final concise card explaining secure off-site IP-camera footage protection. Production migrations `20260718203516_cloud_backup_interest.sql` + `20260718204120_cloud_backup_interest_select_policy.sql`: one consent row per profile (`email`, consent-copy version, timestamp), RLS enabled, anon grant revoked, client SELECT/INSERT/DELETE own with email-match enforcement, admin SELECT only; follow-up consolidates read access into one policy so the performance advisor stays clean. Client can opt in and withdraw; admin client detail shows date + email. Database types regenerated; `rls-pentest.mjs` expanded and passed **83/83** against production |
| 2026-07-18 | **Plan pickers show prices (stakeholder UX round 2).** New `tierOptionLabel()` in `billing.ts` renders every plan option as monthly rate + pipe + plan name (e.g. "$24.95 \| Telephone Land Line"); applied to the create-client form and both client-detail pickers (add-service, plan change). Cadence notes added to the create-form service cards: monitoring is billed annually, VoIP always monthly (per line). Build clean |
| 2026-07-18 | **VoIP stakeholder UX round after first hands-on review.** Every VoIP plan is now per line (`isPerLineService` keys on the service type; a residential customer can carry a fax or second line), `professional` relabeled **"Business"** everywhere (Stripe product renamed to "McKee Security VoIP Business"; the setup script now re-syncs product names/descriptions on re-run, price IDs unchanged). Create-client form rebuilt from a flat grid into sections: Client Details, Services (one bordered card per service type; the phone-lines input lives inside the VoIP card and sits grayed out until a plan is chosen, so nothing pops in and out of the layout), and Billing with "How will they pay?" always visible. Same grayed-until-relevant treatment on the client-detail add-service row. Stakeholder added the two VoIP price env vars to Vercel; entering the two real VoIP customers is deferred to the Stripe go-live by stakeholder choice |
| 2026-07-18 | **Phase 9 (VoIP phone service) executed and gated; runs before Phase 8 by stakeholder direction.** Migrations `20260718190000`/`20260718190100` live on production (enum value in its own migration because new enum values cannot be referenced in the same transaction; `line_count` 1..100; tier CHECK gains VoIP; `admin_create_client` gains defaulted VoIP args so old deploys keep resolving). Code: `VOIP_MONTHLY_CENTS` + shared `planMonthlyCents()` (monitoring-only amount-sync branches in assign/tier-change/billing-save generalized), per-line billing end to end (checkout quantity, webhook quantity sync, admin line editor with live Stripe subscription update, stored amount always rate x lines so reminders/collections/KPIs needed zero changes), VoIP in the create-client form + Clients filters/chips + client detail management + read-only client card. Stripe test-mode products/prices created by the new idempotent `scripts/stripe-voip-setup.mjs` ($34.99 flat, $59.99 per line, monthly CAD); price IDs in `.env.local`, Vercel add is a [HUMAN] item. Gate `scripts/voip-check.mjs` 11/11 against production; build clean. Section 9.5 gained 9.5.6 (VoIP deltas for the accounting rail: VoIP income mapping in D12, seed stays monitoring-only, multi-service reverse-sync matching in the 8C gate); `ACCOUNTING_PLAN.md` revised incl. the audited deliverables list. The two existing VoIP customers will be entered manually by the stakeholder (R42/D14) |
| 2026-07-06 | **Full-system alignment audit against this plan and `PRODUCT_HANDOVER.md` (stakeholder request, end of Track 1).** All eight scripted gates re-run green against a fresh production build: rls-check 8/8, gate-check 5/5, activation-check 24/24, services-check 15/15, caller-id-check 26/26, billing-check 22/22, rls-pentest 73/73, cron-check 24/24. Handover Appendix A checklist verified item by item: every core-portal line is live except the two deliberately deferred areas (footage requests / Arctic = Track 2; QuickBooks = Track 3). Drift found and fixed in this plan: 7.2 tab table gained the Devices tab (R35); Track 3 (QuickBooks Scope B + accounting MCP Scope C) had no implementation section anywhere, now specified in 9.5 with architecture, component placement, build phases QB-1..QB-4, and the Track 3 [HUMAN] kickoff list. Confirmed launch blockers, all stakeholder-side: Stripe is still test mode (`sk_test_`, CLI session key expires 2026-10-03) with no live-mode prices/webhook; real-device mobile + cross-browser pass; production walkthrough sign-off; D11 e-transfer copy confirmation (default in `billing.ts` says info@mckeesecurity.ca / (705) 457-2156); D6 Q1/Q3 remain open-not-blocking. Legacy follow-ups still outstanding from 1.4: rotate the exposed Gmail app password, remove stale DNS for the deleted load balancer, optional Cognito pool deletion |
...
| Date | Decision |
|------|----------|
| 2026-08-13 | R50 (stakeholder / company knowledge 3.12): **VoIP is one monthly figure per system, not per line.** Rate card, one client line, quantity 1, port fee one-time, recurring fully separate from installation. `professional` displayed Commercial. Internal BrightPBX seat cost tracked, DID cost flagged unconfirmed at $0.00. D14 pricing is closed; remaining D14 work is entering the two live customers at Stripe go-live |
| 2026-08-13 | R49 / D15 / D16 (stakeholder): **every monitoring client carries the day they first started monitoring**, separate from portal invitation and activation, so later year-over-year profitability has a real history. Seed infers that date from the earliest QuickBooks monitoring invoice; admin confirms on create, add-service, and import review. **Before any monitoring P&L view**, gather the cost McKee pays per monitored client for each current tier (monitoring-station / communicator cost, not retail) and store it later in a dated rate table in the Starlink `unit_costs` pattern. Posting payments to QuickBooks (8C) does not wait on those costs |
| 2026-07-18 | R42 (stakeholder): **VoIP phone service joins the portal as Phase 9, built before the Phase 8 accounting automation.** Two plans mirroring the two current customers: residential $34.99/mo flat, professional $59.99/mo **per line** (both + HST, billed monthly on either rail); pricing is interim while the offering settles (D14 opened). Per-line billing = `services.line_count` driving the Stripe subscription quantity, with `monthly_amount_cents` always storing the total (rate x lines) so every existing consumer of that column works unchanged. No bulk import for VoIP: two customers, entered manually by the stakeholder. Rationale for the ordering: the monitoring implementation is the template, and shipping the second service type first proves the rails generalize before the accounting rail (which must cover the full catalog, 9.5.6) is built on top |
...
| 2026-07-07 | R41 (stakeholder): **seed fidelity — tier by billed price, contacts from Lanvac, devices from the QB to-do list** (9.5.5A). Tier inference now leads with the billed-amount fingerprint (the four tiers bill at four distinct annual amounts, matched net/gross of HST) corroborated by line-item names; off-price amounts flag as **legacy rate** and the seed preserves what the customer actually pays (re-pricing a grandfathered rate is an explicit admin decision, never an import side effect). Seed enrichment: Lanvac bulk contact-list export (D13) imported into `caller_id_contacts` without client notifications (activation email prompts first-login review, making activation the verification moment), and the bridge mirrors the QB company to-do list (`qb_todos` via ToDoQuery) whose notes parse into prefill-only device drafts. A per-client migration checklist (imported / contacts / devices / invited / activated) on client detail and the Billing tab makes partial migration visible instead of memory-dependent |
| 2026-07-07 | R40 (stakeholder): **migration outreach mechanics + same-payment collision guard** (9.5.5). Outreach: cron-built invite queue (seeded clients entering the renewal window), one-click combined renewal + activation email with legacy payment instructions and autopay pitch, reply-to `info@` plus an admin copy of every send, invited-vs-activated tracked per client; activation never requires leaving the manual rail, autopay is encouraged in copy only. Collision guard: the one race echo suppression cannot catch is the same cheque entered in both systems independently; both sync directions proximity-check (same customer, same amount, +/- 7 days) and park `needs_review` on a hit, so a double entry becomes a one-click human resolution, never a double posting. Origination hierarchy stated plainly: Stripe payments always originate in the portal; legacy payments preferably in the portal with direct-to-QB as a supported fallback |
| 2026-07-07 | R39 (stakeholder): **bulk alignment seed + payment reverse-sync** (9.5.5). (A) The migration cohort is seeded from QuickBooks in bulk: mirrors + invoice line-item history prefill draft profiles (tier, amount, projected next due), admin confirms each in a staging review, commit creates linked pending profiles + manual-rail services with no invitations sent; idempotent re-runs. The collections console then covers the whole business from day one and clients are invited in renewal waves ("activate before your next payment"), so portal activation is a client-experience upgrade, never a bookkeeping prerequisite. (B) Payments keyed directly into QuickBooks Desktop flow back: bridge incremental ReceivePaymentQuery, echo suppression via TxnIDs (portal-originated postings can never bounce back and double-advance due dates; imports unique on `manual_payments.qb_txn_id`), clean matches auto-apply through the recordManualPayment path, ambiguity parks needs_review. Scope fence: **payments-only** bidirectionality; customer records stay one-way mirror + explicit tasks, full two-way record sync deliberately rejected |
| 2026-07-07 | R38 (stakeholder): **duplicate prevention for the existing-customer migration is a first-class Phase 8 requirement** (9.5.4). Most onboarding will be existing customers who already exist in QuickBooks, so 8C's earlier "customer.create on first sight of a payment" draft is deleted: posting now requires an explicit one-to-one link (`qb_customers.profile_id`, unique), QB customer creation is an explicit approval-gated admin choice only, and unlinked payments park in `needs_review` with normalized match suggestions (email/phone/name) instead of guessing. The migration cohort gets a QB-first "Onboard from QuickBooks" flow (profile born linked from the mirror row); portal-first profiles get a link control on client detail; the 8C backfill doubles as the link-completeness sweep. Portal side: create-client gains a near-duplicate warning (exact email/phone, close name) on top of the existing unique-email index |
