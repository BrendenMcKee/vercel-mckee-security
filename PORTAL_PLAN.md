...
- [ ] **[HUMAN]** Custom SMTP for auth emails: `RESEND_API_KEY` is sensitive in Vercel (not pullable), so place it in `website/.env.local`, then run `powershell -File website/scripts/configure-auth-smtp.ps1`. Until then, recovery emails use the built-in mailer (unbranded sender, 2/hour cap)
