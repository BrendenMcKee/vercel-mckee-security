...
**Client-facing billing mail moves entirely to the portal (stakeholder 2026-07-07).** The legacy practice of emailing monitoring invoices from QuickBooks ends at Phase 8: renewal reminders, autopay receipts, payment confirmations, and migration invitations are all portal-sent (9.4, R40). QuickBooks documents are **sales receipts** created by the automation after the portal has collected the money (D12, locked 2026-08-16), never customer communication. Fixed constraints from the handover (23.13) remain non-negotiable: QuickBooks Desktop Canada stays the system of record, no unrestricted AI or website writes to QuickBooks, offline-safe queue, approval gates for posting, full audit trail.
...
2. Committing creates linked `pending` profiles + manual-rail services with true due dates. **No invitations are sent at import.** Re-running the import is idempotent: already-linked mirror rows are skipped, so the seed can never duplicate.
3. **The migration then runs in renewal waves off the Billing tab.** Because every seeded customer's `next_due_on` is real, the collections console covers the entire business from day one, before any client has touched the portal. As each renewal approaches, the admin sends that client's invitation ("activate your account before your next payment"): the client activates, optionally switches to autopay (R29), and their first portal-era payment posts to QuickBooks through 8C. Clients who never activate still get reminder emails, still show on the collections board, and their recorded payments still post to the books: portal activation is a client-experience upgrade, not a bookkeeping prerequisite.
...
5. Freshness bound: QB-keyed payments appear in the portal within one bridge poll cycle while the office PC is on, or when it next comes online. The daily reminder cron tolerates that race: worst case a reminder goes out the same day the cheque is keyed, and the import clears the guard immediately after.
