...
**Next product work:** R53 slices 4–6 (admin two buttons / Account card attach, client switcher, emails). Slices 1–2 plus the 2026-08-28 audit shipped. Slice 3 (2026-08-29): last Account admin cannot be revoked, and delete confirm copy names **this site**. Hosted two-site fixture (do not delete): McKee account, Bunkie `O5985` + House `O4964`. Then the CUA playbook ([`docs/PORTAL_CUA_TEST.md`](./docs/PORTAL_CUA_TEST.md)) before the Windows bridge or real import. R54 is done. Client mail stays off (R52 / 9.5.5C). When a slice changes UI or schema, update those docs in the same commit so implementation is never working from a stale page.
