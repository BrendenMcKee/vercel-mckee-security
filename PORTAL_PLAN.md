...
| R46 | Admin sessions on the client portal are a wrong-door, not a client dashboard | Stakeholder 2026-08-13: an active admin who signs in at `/user-dashboard` sees a staff-only message and a link to `/admin-dashboard`. Unused-service promo cards are never shown to staff. Client and admin sign-in chrome are visually distinct (client: shield + brand red; staff: amber console + "Staff only") so the two doors cannot be mistaken for each other |
...
| Client detail | one page per client: profile info, services (assign/modify tier, cancel/restart, set billing method, amount, next due, **monitoring `started_on` distinct from invitation/activation**, R49; each service row carries its product color), payment history (both rails), **Camera Cloud Backup interest confirmation (date + consented email, R43)** plus a separate grayed **In Development** retention template (R44; 7/30/90 shown but disabled), devices and caller IDs only when R45 applies (view **and edit**, R23; the editor requires an authorization method + reason note before save and shows the diff in the confirm dialog, R24) + change history, invitation state + resend | 3 core, 4 caller ID editor, 5 billing fields |
