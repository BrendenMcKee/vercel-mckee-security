...
**Multi-site accounts (planned, not built):** [`docs/MULTI_SITE_ACCOUNTS.md`](./docs/MULTI_SITE_ACCOUNTS.md) is the locked model. **R53** (this register) is the portal-plan pointer. After that ships, a computer-using agent runs [`docs/PORTAL_CUA_TEST.md`](./docs/PORTAL_CUA_TEST.md) before the Windows bridge or real import. Client mail stays off (R52 / 9.5.5C). When a slice changes UI or schema, update those docs in the same commit so implementation is never working from a stale page.
...
1. Multi-site accounts have shipped (R53) and the CUA playbook ([`docs/PORTAL_CUA_TEST.md`](./docs/PORTAL_CUA_TEST.md)) is clean or every fail is accepted.
...
- [ ] **Multi-site accounts (R53).** Locked model in [`docs/MULTI_SITE_ACCOUNTS.md`](./docs/MULTI_SITE_ACCOUNTS.md). Two slices then an audit. First stop: schema + `resolvePortalSession` / orphan / OAuth / cleanup / password. Do not start the Windows bridge or real import in this item.
