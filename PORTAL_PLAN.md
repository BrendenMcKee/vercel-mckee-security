...
Email presentation amendment (stakeholder 2026-07-18): the HTML header uses the real McKee shield mark (`/images/favicon-192.png`) instead of platform-dependent emoji artwork, and all green CTA buttons render clean text with **no appended arrow**. The plaintext fallback keeps a restrained semantic character where useful. The client invitation leads with the benefit ("one place to review services, manage billing, and keep account information current"), says setup takes about a minute, and avoids system-generated language.
