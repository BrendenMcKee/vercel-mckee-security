...
| R44 | Camera Cloud Backup admin controls remain visible but locked until Track 2 | Stakeholder 2026-07-18: both admin service-assignment surfaces keep a separate, grayed-out Camera Cloud Backup template marked **"In Development"**. It previews the established 7-/30-/90-day retention choices (R19; these map to the current storage-class design), but the select is disabled and normal create-client/add-service server actions reject cloud assignment. `SERVICE_LAUNCH_STATUS.cloud_backup` is the single application gate. It flips to available only after Phase 6A/6B launch gates pass, so a visual placeholder can never be mistaken for a working service |
...
|---------|---------|----------------|----------------|
| Security Monitoring | tier + status badge when assigned. If absent, a **secondary** dashed/muted card lives in an "Other McKee services" band below billing, never mixed with owned services | **none** (read-only); absent clients can call McKee | Handover 6.2. Unused services are visually demoted (stakeholder 2026-08-13). Product color: brand red |
