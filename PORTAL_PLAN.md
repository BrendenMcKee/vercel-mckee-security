...
| D13 | Lanvac contact-list export + API + QB to-do device source | Phase 8A (seed), portal caller-ID save (API) | **Lanvac:** full Excel export in hand (2026-08-14 / confirmed 2026-08-16). One-time seed into `caller_id_contacts` (admin-attributed, no client emails). After cutover, a list save still emails McKee and the customer, **and** writes the list to Lanvac over their API so staff do not RDP to retype. API failure does not roll back the portal save; emails still go; RDP is the fallback. Ingest the API contract (auth, account key, write shape) before building; no keys in git. **Devices:** the QB company To Do list (841 notes) is a **one-time source**, not an ongoing system. After the 8A seed, portal `devices` is the system of record; staff stop using QB To Dos for battery/smoke tracking and we do **not** two-way sync them. Samples are in `ACCOUNTING_GATHER_DATA.md`. Parser: AI-assisted draft with confidence; original note kept; low-confidence is a human queue. Not a blocker for 8C posting. |
...
- **Caller ID lists come from Lanvac** (the monitoring station holds the authoritative contact lists and passcodes). The full Excel export is in hand. A one-time importer writes `caller_id_contacts` per matched client through the existing save path, attributed as an admin import with **no client diff notifications during seeding**. After cutover, every list save still emails McKee and the customer (audit trail unchanged) and **also writes the list to Lanvac's API** so staff do not retype it over remote desktop. If the API call fails, the portal save and emails still succeed; RDP is the fallback. The activation invitation asks the client to **review their alarm contact list on first login**.
...
- [ ] **[HUMAN]** D13: full Lanvac Excel is in hand; remaining work is matching it to portal clients at seed time, then ingesting the Lanvac API contract (no keys in git) so a list save writes the station. Device samples are in `ACCOUNTING_GATHER_DATA.md`. Stakeholder reviews low-confidence device drafts when the import screen is ready
