...
| D12 | QuickBooks host machine + bookkeeper GL mapping | Phase 8A (machine), Phase 8C (mapping) | **[HUMAN]** Confirm the canonical QuickBooks Desktop Canada PC (handover Q20) and its release (SDK 17 needs 2023 R16+ / 2024 R18+); approve the bridge install and whether the PC can stay on more regularly. File hygiene (D17 / 9.5.7) is part of this kickoff. Before 8C posting: bookkeeper session for the chart-of-accounts mapping (Q18: Stripe Clearing, Stripe Fees, HST/GST Payable, Monitoring Income, VoIP Income) and the invoice-first vs sales-receipt posting model |
| D13 | Lanvac contact-list export + QB to-do device source | Phase 8A (seed enrichment, R41) | **Lanvac:** export received 2026-08-14; importer vs. hand-entry still depends on how cleanly it matches portal clients. **Devices:** the QB company To Do list (841 notes) is a **one-time source**, not an ongoing system. After the 8A seed, portal `devices` is the system of record; staff stop using QB To Dos for battery/smoke tracking and we do **not** two-way sync them. Samples are already in `ACCOUNTING_GATHER_DATA.md`. The full list is pulled by the bridge (`ToDoQuery`) when 8A is built, not during the bookkeeper sitting. Parser: AI-assisted draft into the existing device shape (label, category, `installed_on`, `lifetime_years`, matched customer) with a confidence score; original note always kept on the draft; high-confidence rows are still reviewed in staging; low-confidence rows are a human queue. Not a blocker for bookkeeper mapping or 8C posting. |
...
| Local bridge | Office QB PC (Windows service) | C#/.NET + Desktop SDK/qbXML. **Not Web Connector** (TSheets already owns that slot on the live file; 9.5.7). Validates payloads against **named commands only** (`receive_payment.create`, `customer.query`...); syncs mirrors on an interval; never accepts free-form writes. Opens only `expected_company_file` |
| Task enqueueing | Vercel (webhook + admin actions) | Stripe `invoice.paid` already lands in `billing_events`; Scope B adds an enqueue step mapping it to a `receive_payment.create` task. `billing_events` retention means history can be backfilled from day one |
...
- **Caller ID lists come from Lanvac** (the monitoring station holds the authoritative contact lists and passcodes). D13 requests a bulk export of every account's list (names, phones, passcodes, call order). If the export is machine-readable, a one-time importer writes `caller_id_contacts` per matched client through the existing save path, attributed as an admin import with **no client diff notifications during seeding**; if not, contacts are entered manually per client. Either way the migration checklist (below) tracks which clients have their list in. The activation invitation asks the client to **review their alarm contact list on first login**, which makes activation the natural verification moment for imported data.
...
- **Nothing structural changes.** The enqueue paths (Stripe webhook, `recordManualPayment`), the task queue, echo suppression, and the collision guard are all keyed on `service_id` / customer / amount, not on service type. A VoIP payment posts through the identical `receive_payment.create` path.
- **Bookkeeper mapping (D12) gains VoIP lines.** The 8C session must map **VoIP Income** (or the existing item(s) used for VoIP invoices today) alongside Monitoring Income, and confirm the QB item names used on the two current customers' VoIP invoices, so postings and any invoice-creation tasks are itemized correctly.
...
- [ ] **[HUMAN]** D17: company file hygiene (9.5.7). Inventory `.qbw` files. Leave the live filename as TSheets knows it. Restore a backup as `…PORTAL-TEST do-not-invoice.qbw` in a different folder. Change the copy's Company Name. Confirm TSheets still syncs on live and is not authorized on the copy. Record both full paths
...
- [ ] **[HUMAN]** D13: Lanvac export is in (2026-08-14); remaining work is matching it to portal clients at seed time. Device samples are in `ACCOUNTING_GATHER_DATA.md`. Stakeholder reviews low-confidence device drafts when the import screen is ready; no full To Do export and no bookkeeper input required for this path
...
- [ ] **[HUMAN]** D12 (mapping): bookkeeper session for the chart of accounts (Stripe Clearing, Stripe Fees, HST/GST Payable, Monitoring Income, **VoIP Income + the QB item names on the two current VoIP invoices**, 9.5.6) and the invoice-first vs sales-receipt posting model (handover 23.8: gross payment, fees separate, clearing account reconciles the payout); then flip `qb_bridges.mode` to `live`, set `expected_company_file` to the live path, and reopen the live file. Sandbox postings in the copy are not replayed onto live
- [ ] Stripe rail enqueue: the `invoice.paid` webhook handler also enqueues `receive_payment.create` against the **linked** QB customer (9.5.4) with amounts split per the approved mapping; unlinked profiles land the task in `needs_review` with match suggestions attached, and nothing posts or creates
- [ ] Manual rail enqueue: `recordManualPayment` (7.3) enqueues the equivalent task, so e-transfer/cheque/cash payments post too (same link requirement)
