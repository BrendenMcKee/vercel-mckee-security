...
- **Webhook `/api/stripe/webhook`:** raw body via `await req.text()`, `stripe.webhooks.constructEvent` signature verification. Insert into `billing_events` first (PK conflict = already processed, return 200). Handle: `checkout.session.completed` (service -> `active`, store subscription id **and stamp `next_due_on` from the subscription's period end**), `customer.subscription.updated` (tier/status/`next_due_on` sync), `customer.subscription.deleted` (-> `cancelled`), `invoice.paid` (renewal: advance `next_due_on`, client payment history row; falls back to subscription metadata when it outraces checkout.session.completed), `invoice.payment_failed` (-> flag + admin email). All writes via service role. **The Stripe endpoint's enabled-events list must include all five** (R33: `invoice.paid` was missing at first setup).
