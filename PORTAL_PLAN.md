...
| D13 | Lanvac contact-list export + QB to-do device source | Phase 8A (seed enrichment, R41) | **[HUMAN]** Request a bulk export of every account's caller ID list from Lanvac (names, phones, passcodes, call order); the format decides importer vs. manual entry. Confirm the QB company to-do list is the canonical device/battery record and provide sample entries (wording, dates) to calibrate the device-draft parser |
...
| `qb_todos`: `todo_id` text PK, `notes` text, `is_done` boolean, `reminder_date` date nullable, `synced_at` timestamptz (mirror of the QB company to-do list via `ToDoQuery`; source for device/battery drafts in the 9.5.5A seed, R41) |
...
- **Device/battery statuses come from the QuickBooks company to-do list**, where they are tracked today. The bridge adds `ToDoQuery` to its mirror pass (`qb_todos`), and the seed parses to-do notes into **device drafts** (label, category, install/replace date, matched customer) that ride the same staging review as everything else. To-do notes are freeform text, so parsing is heuristic and prefill-only; the D13 sample entries calibrate the parser before the real run.
...
- **The bulk alignment seed (9.5.5A) stays monitoring-only.** There are two VoIP customers; the stakeholder enters them manually (R42/D14) and links them to their QB customers with the standard 9.5.4 link control. The tier-inference fingerprint is not extended to VoIP amounts: the 3.12 derived totals are not a small closed set, and two manual entries do not justify one.
...
- **Existing customers:** exactly two (one residential, one commercial). No import tooling: the stakeholder creates them manually via the create-client flow, **deferred by stakeholder choice (2026-07-18) until the Stripe go-live**, and links them to QuickBooks in Phase 8A.
...
- [ ] Seed enrichment (R41): bridge `ToDoQuery` mirror of the QB company to-do list (`qb_todos`); to-do notes parsed into device drafts (label, category, date, matched customer) in the same staging review; Lanvac caller ID import per D13 (one-time importer if the export is machine-readable, manual entry otherwise; admin-attributed, no client notifications during seeding); per-client migration checklist (imported / `started_on` confirmed / alarm contacts entered / devices entered / invited / activated) on client detail + Billing-tab filter
- [ ] **[HUMAN]** D13: request the bulk contact-list export from Lanvac (names, phones, passcodes, call order for every account); confirm the QB to-do list is the canonical device/battery record and provide sample entries so the parser is calibrated against real wording
