...
Global chrome: site `Header`/`Footer` stay (portal is native to the site); welcome header "Welcome, {first_name}", sign-out, then Dashboard / Settings / Alerts tabs (R47). Loading skeletons per card, per-section error boundaries with retry (handover 22.3).
...
| VoIP Phone Service | plan + monthly total + coverage (numbers, seats on commercial) + status badge when assigned; unused VoIP is the same muted secondary card as unused monitoring | **none** (read-only; number/seat/plan changes go through McKee) | R42/R50. Product color: teal, coherent on client cards, admin chips, and overview |
