...
| R42 | VoIP phone service is a first-class portal service (Phase 9) | Stakeholder 2026-07-18: VoIP joins monitoring on the full billing rails (Stripe autopay + manual ledger + reminders + collections) so it too can take automatic payments. `service_type` gains `'voip'` with two plans matching the two current offerings: `residential` (Residential Unlimited Canada-Wide, $34.99/mo + HST) and `professional` (displayed as **"Business"**, $59.99/mo + HST). **Amended same day: every VoIP plan is per line** (a residential customer can carry a second line, e.g. a fax), so `services.line_count` applies to both plans (Stripe subscription quantity = lines; stored amount = rate x lines). Pricing is interim while the tier structure settles (D14): only two customers exist today, so no bulk import; the stakeholder enters both manually, **deferred to the Stripe go-live by stakeholder choice**. VoIP bills **monthly** on both rails, unlike monitoring's annual invoicing. Built before Phase 8 so accounting automation covers the whole catalog |
...
| D14 | VoIP tier structure + final pricing | Revisit before marketing VoIP broadly | **[HUMAN]** The two current plans (residential $34.99/line, Business $59.99/line, both monthly + HST, every plan per line) are interim (R42). When the offering firms up (more tiers, feature splits), the change is: extend the tier CHECK + `SERVICE_TIERS`/`VOIP_MONTHLY_CENTS` maps + new Stripe prices, same as any catalog change. Also **[HUMAN]**: enter the two existing VoIP customers manually (stakeholder chose manual entry over import; deferred to the Stripe go-live, 2026-07-18) |
...
**VoIP plans** (stakeholder 2026-07-18, R42; interim pricing while the tier structure settles, D14; **per month plus tax, billed monthly** on both rails; **every plan is per line**: `line_count` multiplies the rate and sets the Stripe subscription quantity):

| Tier key | Displayed as | Monthly per line (pre-tax) | Notes |
|----------|--------------|----------------------------|-------|
| `residential` | Residential Unlimited Canada-Wide | $34.99 | Unlimited Canada-wide calling, all features included. Extra lines (e.g. a fax line) just raise `line_count` |
| `professional` | Business | $59.99 | Business lines. Stripe product renamed "McKee Security VoIP Business" to match |

VoIP prices live in `billing.ts` (`VOIP_MONTHLY_CENTS`; `planMonthlyCents()` is the shared lookup); assigning or changing a VoIP plan prefills `monthly_amount_cents` as rate x lines.
...
| `line_count` | integer | not null default 1, CHECK 1..100 (R42); per-line plans (every VoIP plan): Stripe subscription quantity = lines, and the stored amount is rate x lines. Non-VoIP services keep 1 |
| `monthly_amount_cents` | integer | nullable; the **monthly pre-tax total** (an annual invoice = 12x + tax; for per-line plans this is rate x `line_count`); required when `billing_method='manual'` (reminder emails and the Billing tab quote it) |
...
- **Model:** one Stripe Product per service type. Monitoring: one recurring Price per tier (annual CAD, 12x monthly). VoIP: one recurring monthly Price per plan (R42); every VoIP subscription item carries `quantity = line_count` (all plans are per line), so per-line billing is native Stripe quantity, not a custom price. Cloud backup: one recurring **per-camera** Price per (retention tier, resolution) cell (9 prices, matrix in 9.2.7); the subscription carries an item per cell in use with quantity = camera count. Price IDs live in a server-side map in `lib/portal/stripe.ts`, keyed by `(service_type, tier, resolution)`. Client code never sends price IDs.
...
- **The bulk alignment seed (9.5.5A) stays monitoring-only.** There are two VoIP customers; the stakeholder enters them manually (R42) and links them to their QB customers with the standard 9.5.4 link control. The tier-inference fingerprint is not extended to VoIP amounts: with per-line multiples and interim pricing there is no stable fingerprint, and two manual entries do not justify one.
- **Reverse-sync matching now spans multiple services per client.** A client can hold monitoring and VoIP; a QB payment matches by amount against each candidate service's expected invoice (monitoring: monthly rate x 12; VoIP: rate x lines x 1). Distinct amounts and intervals make clean matches the norm; anything ambiguous already parks `needs_review` (9.5.5B rule 3), so no new mechanism is needed, just test coverage in the 8C gate.
...
### 9.6 VoIP phone service (Phase 9; R42)

VoIP is deliberately a thin phase: it is the monitoring template applied to a second service type, which is exactly what proves the rails are reusable before Track 2 needs them.

- **Catalog:** two plans (4.1), **both per line** (amended 2026-07-18: a residential customer can carry a fax or second line): `residential` ($34.99/mo/line) and `professional` (displayed **"Business"**, $59.99/mo/line). Interim pricing (D14). Labels and tier lists live in `service-labels.ts`; rates in `billing.ts` (`VOIP_MONTHLY_CENTS`, shared `planMonthlyCents()` lookup); `isPerLineService()` keys on the service type alone.
- **Per-line billing:** `services.line_count` (default 1, CHECK 1..100). The stored `monthly_amount_cents` is always the **total** (rate x lines) so every existing consumer (reminder emails, collections board, KPIs, dashboard displays) works unchanged. On the Stripe rail the subscription item's `quantity` is the line count; checkout sends it, the admin line-count action updates it (`proration_behavior: none`), and the webhook syncs it back if it ever changes Stripe-side.
...
- **Stripe:** two products + monthly CAD prices created in test mode via `scripts/stripe-voip-setup.mjs` (idempotent, metadata-marked, re-runnable for the live-mode go-live). Env vars `STRIPE_PRICE_VOIP_RESIDENTIAL` / `_PROFESSIONAL` in the same `PRICE_ENV_KEYS` map; the existing HST tax rate applies at checkout unchanged.
- **Admin:** the create-client form is sectioned (stakeholder UX round, 2026-07-18): Client Details, then a Services fieldset with one bordered card per service type (monitoring — plan plus a required **Monitoring started** date when a plan is chosen, R49; VoIP with its plan select and an always-visible lines input that is grayed out until a plan is chosen, cloud backup), then Billing with "How will they pay?" **always visible** rather than appearing only once a service is picked. Nothing pops in or out of the layout. Every plan picker (create form, add-service row, plan-change select) shows the monthly rate first: `"$34.99 | Residential Unlimited Canada-Wide"` (`tierOptionLabel()` in `billing.ts`; pre-tax, HST understood). Cloud backup remains its own card but is deliberately grayed, marked **In Development**, and disabled under R44; its 7/30/90 retention template stays visible until Track 2 unlocks it. Each active service card carries its cadence note: monitoring "Monthly rate, billed annually (one invoice a year)", VoIP "Monthly rate per line, always billed monthly" (a many-line business must never get one giant annual bill). `admin_create_client` RPC extended with optional `p_voip_tier`/`p_voip_lines` (defaulted, so older deploys keep working); client detail Services & Billing card manages active plans, lines, status, and rails identically to monitoring (same grayed-until-relevant lines input on the add-service row); Clients tab filters by VoIP service and plan.
- **Client:** a read-only VoIP card (plan, rate x lines, "call McKee to change"); the shared Billing & Payments card carries amount, next payment, autopay setup, and history with zero VoIP-specific code.
...
- **Gate:** `scripts/voip-check.mjs` (schema round-trip incl. per-line math and CHECK rejections, RPC with VoIP args, Stripe price verification) plus the full build. Marketing-site VoIP pages are unchanged; this phase is portal + billing only.
...
- [ ] **[HUMAN]** Enter the two existing VoIP customers manually: residential at $34.99/mo with their real line count, commercial on Business with theirs; link both to QuickBooks in Phase 8A. **Deferred by stakeholder (2026-07-18) to the Stripe go-live**
- [ ] **[HUMAN]** At Stripe go-live: re-run `stripe-voip-setup.mjs` with the live key and put the live price IDs in Vercel alongside the monitoring ones (Phase 8C coupling, 9.5.3)

**Gate:** scripted checks green (done); a VoIP client on autopay checks out with the correct quantity and total in Stripe test mode; a line-count change updates the subscription quantity and the next invoice; a VoIP manual payer gets reminders quoting rate x lines. The remaining [HUMAN] items do not block Phase 8A.
...
| VoIP takes automatic payments on the same rails as monitoring (stakeholder 2026-07-18, R42) | Phase 9: `voip` service type + per-line `line_count`, Stripe quantity billing, shared reminder/collections machinery (4.2, 9.6); posts to the books via Phase 8 like every service (9.5.6) |
...
| `STRIPE_PRICE_VOIP_RESIDENTIAL/_PROFESSIONAL` | Vercel only | 9 — set (test-mode monthly CAD per-line prices, `stripe-voip-setup.mjs`; in Vercel 2026-07-18) |
