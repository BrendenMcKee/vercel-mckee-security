...
| R42 | VoIP phone service is a first-class portal service (Phase 9) | Stakeholder 2026-07-18: VoIP joins monitoring on the full billing rails (Stripe autopay + manual ledger + reminders + collections) so it too can take automatic payments. `service_type` gains `'voip'` with two plans matching the two current offerings: `residential` (Residential Unlimited Canada-Wide, $34.99/mo + HST, flat) and `professional` (VoIP Professional, $59.99/mo + HST **per line**). Pricing is interim while the tier structure settles (D14): only two customers exist today, so no bulk import; the stakeholder enters both manually. Per-line billing is a `services.line_count` column (Stripe subscription quantity = lines; stored amount = rate x lines). VoIP bills **monthly** on both rails, unlike monitoring's annual invoicing. Built before Phase 8 so accounting automation covers the whole catalog |
...
| D14 | VoIP tier structure + final pricing | Revisit before marketing VoIP broadly | **[HUMAN]** The two current plans (residential $34.99, professional $59.99/line, both monthly + HST) are interim (R42). When the offering firms up (more tiers, feature splits, per-line vs flat), the change is: extend the tier CHECK + `SERVICE_TIERS`/`VOIP_MONTHLY_CENTS` maps + new Stripe prices, same as any catalog change. Also **[HUMAN]**: enter the two existing VoIP customers manually (stakeholder chose manual entry over import) and add the two `STRIPE_PRICE_VOIP_*` env vars to Vercel (Phase 9 checklist) |
...
**VoIP plans** (stakeholder 2026-07-18, R42; interim pricing while the tier structure settles, D14; **per month plus tax, billed monthly** on both rails):

| Tier key | Product | Monthly (pre-tax) | Notes |
|----------|---------|-------------------|-------|
| `residential` | Residential Unlimited Canada-Wide | $34.99 | Flat; unlimited Canada-wide calling, all features included |
| `professional` | VoIP Professional | $59.99 **per line** | `line_count` multiplies the rate and sets the Stripe subscription quantity |
...
| `line_count` | integer | not null default 1, CHECK 1..100 (R42); per-line plans only (VoIP professional): Stripe subscription quantity = lines, and the stored amount is rate x lines. Flat plans keep 1 |
...
- **Model:** one Stripe Product per service type. Monitoring: one recurring Price per tier (annual CAD, 12x monthly). VoIP: one recurring monthly Price per plan (R42); the professional plan's subscription item carries `quantity = line_count`, so per-line billing is native Stripe quantity, not a custom price. Cloud backup: one recurring **per-camera** Price per (retention tier, resolution) cell (9 prices, matrix in 9.2.7); the subscription carries an item per cell in use with quantity = camera count. Price IDs live in a server-side map in `lib/portal/stripe.ts`, keyed by `(service_type, tier, resolution)`. Client code never sends price IDs.
...
- **Catalog:** two plans (4.1): `residential` ($34.99/mo flat) and `professional` ($59.99/mo **per line**). Interim pricing (D14). Labels and tier lists live in `service-labels.ts`; rates in `billing.ts` (`VOIP_MONTHLY_CENTS`, shared `planMonthlyCents()` lookup).
...
- **Admin:** create-client form gains a VoIP plan select (+ lines input when professional); `admin_create_client` RPC extended with optional `p_voip_tier`/`p_voip_lines` (defaulted, so older deploys keep working); client detail Services & Billing card manages plan, lines, status, and rails identically to monitoring; Clients tab filters by VoIP service and plan.
...
- **Existing customers:** exactly two (one residential, one commercial). No import tooling: the stakeholder creates them manually via the create-client flow when ready (D14), on the manual rail or straight to autopay, and links them to QuickBooks in Phase 8A.
...
- [ ] **[HUMAN]** Add `STRIPE_PRICE_VOIP_RESIDENTIAL` and `STRIPE_PRICE_VOIP_PROFESSIONAL` to Vercel (values printed by the setup script, also in `.env.local`)
- [ ] **[HUMAN]** Enter the two existing VoIP customers manually when ready (D14): residential at $34.99/mo, commercial on professional with their real line count; link both to QuickBooks in Phase 8A
...
| `STRIPE_PRICE_VOIP_RESIDENTIAL/_PROFESSIONAL` | Vercel only | 9 — test-mode monthly CAD prices created 2026-07-18 (`stripe-voip-setup.mjs`); in `.env.local`, **[HUMAN] add to Vercel** |
