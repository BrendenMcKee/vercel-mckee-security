...
- **Stripe:** `scripts/stripe-voip-setup.mjs` creates/updates the two plan products (names re-synced to Residential / Commercial) plus the Number Port Fee product. Catalog env vars: `STRIPE_PRICE_VOIP_RESIDENTIAL` / `_PROFESSIONAL` (base monthly CAD) and `STRIPE_PRICE_VOIP_NUMBER_PORT` (one-time CAD). The existing HST tax rate applies at checkout unchanged.
...
- [ ] **[HUMAN]** Add `STRIPE_PRICE_VOIP_NUMBER_PORT` to Vercel (test mode, after the 2026-08-13 setup re-run). At Stripe go-live: re-run `stripe-voip-setup.mjs` with the live key and put the live price IDs (including the port fee) in Vercel alongside the monitoring ones (Phase 8C coupling, 9.5.3)
...
| `STRIPE_PRICE_VOIP_RESIDENTIAL/_PROFESSIONAL` | Vercel only | 9 — set (test-mode monthly CAD **base** prices, `stripe-voip-setup.mjs`; in Vercel 2026-07-18). Amounts above the base create extra Prices on the same product |
| `STRIPE_PRICE_VOIP_NUMBER_PORT` | Vercel only | 9 — one-time $49.99 CAD per number ported. Created by the 2026-08-13 setup re-run. **[HUMAN]** add to Vercel |
