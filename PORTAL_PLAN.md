...
| `port_count` | integer | not null default 0, CHECK 0..100 (R50). How many of those numbers are being ported. Drives the one-time $49.99 port fee only. Never part of `monthly_amount_cents` |
...
- **Port fee:** $49.99 one-time per number in `port_count`. Charged via `chargeVoipPortFee` as a one-time Stripe invoice (autopay) or a ledger row that does **not** advance `next_due_on` (manual). Never a subscription line, never inside an installation invoice.
...
- **Billing interval:** monthly on both rails (services default), unlike monitoring's annual invoicing. The interval-aware machinery from R26 handles both without special cases.
