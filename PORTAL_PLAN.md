...
1. Admin selects mirror customers (or "all active monitoring customers"). For each, the system builds a **draft**: name and email from the mirror; monitoring tier, amount, and `next_due_on` **inferred** from mirrored invoice history (line-item names map to tiers; last invoice date + interval projects the next due date). Inference only prefills; nothing commits without the admin walking the staging review and confirming or correcting each row.
...
- [ ] Bulk alignment seed (9.5.5A): "Import from QuickBooks" staging review — drafts prefilled from mirrors (name, email) and invoice history (tier from line-item names, amount, projected `next_due_on`), admin confirms/corrects each row, commit creates linked `pending` profiles + manual-rail services, **no invitations sent**; idempotent re-runs (linked rows skipped). `qb_invoices` mirror carries line items for the inference
- [ ] Migration outreach (R40): cron-driven invite queue on the Billing tab (seeded, never-invited, renewal inside reminder window); one-click combined renewal + activation email (amount due, legacy payment instructions, activation link, autopay pitch); reply-to `info@mckeesecurity.ca` + admin copy on every send; invited-at vs. activated-at visible on client rows and in the collections digest
- [ ] **[HUMAN]** stakeholder runs the seed against the real mirror list and spot-checks a sample of drafts (tiers, amounts, due dates) before commit

**Gate 8A:** mirrors match the sample file exactly; the bridge survives a reboot and a QuickBooks-closed state gracefully; the portal is fully functional with the bridge offline; bridge secret rotation works; onboarding from a mirror row creates a pre-linked profile and the same row cannot be onboarded twice; a duplicate create attempt (same email, same phone, near-identical name) triggers the warning; the bulk seed round-trips correctly (seeded count = selected count, re-run creates zero new rows, every seeded client lands on the Billing tab with a real due date).
