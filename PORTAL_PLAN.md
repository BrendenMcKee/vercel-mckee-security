...
| `lanvac_city` | text | nullable in the column; 1..240. Must be an exact key from the live Lanvac emergency-numbers directory (`policeNumbersCity`). Admin dropdown: McKee-frequency first, then the rest of the ~3964 directory cities (`lanvac-cities.ts`). Guessed from service address when possible; admin can override. Police/fire/ambulance stay station-owned and are shown read-only from that city. |
...
- [x] **[HUMAN]** D13 API contract ingested and write-tested 2026-08-16 on `O5985`. Portal station fields shipped (`lanvac_account_code`, `lanvac_city`). Live directory is the city dropdown. Vercel env set; thank-you email sent. Remaining: match Excel CODEs to portal clients at seed time; wire `fullupdate` on list save (not yet). Device samples are in `ACCOUNTING_GATHER_DATA.md`.
