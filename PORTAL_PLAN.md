...
**Organization grouping (R53, before invitations).** **Board shipped 2026-09-07** (`/admin-dashboard?tab=grouping`). Lanvac Excel has no org id and no email. Seed one pending site per CODE / QB customer. The grouping board on existing profiles (and later on seeded rows) suggests likely groups (exact NAME on 2+ CODEs, civic keywords as a watchlist, same contact email, QB `parent_list_id` when present). Accept / reject only. Never auto-attach. Human sign-off writes `portal_settings.org_grouping_reviewed_at` and is a GO LIVE gate (empty-queue sign-off is allowed so the flip is not blocked before the first import). GO LIVE also refuses if any suggestion is still open. Attach expires leftover per-site invites and turns `auto_onboard` off. See [`docs/MULTI_SITE_ACCOUNTS.md`](./docs/MULTI_SITE_ACCOUNTS.md).
...
- [ ] **Multi-site accounts (R53) remaining slices.** Still open: Appoint account admin + attach/move, client switcher, emails. Grouping board shipped 2026-09-07.
