...
**8A first migration slice (applied 2026-08-17 on hosted):** `supabase/migrations/20260817143259_qb_bridge_foundation.sql` (hosted version `20260817143259`). Tables: `qb_bridges`, `qb_customers`, `qb_invoices`, `qb_payments`, `qb_todos`. Alter: `services.started_on date`. RLS: admin SELECT on all five; no client policies; no INSERT/UPDATE/DELETE for `authenticated` (bridge routes use service role). Unique index on `qb_customers.profile_id` where not null. Types regenerated. **Cloud routes (2026-08-17):** `/api/qb/poll`, `/api/qb/report`, `/api/qb/mirror` are live in the website app. **Still not built:** remaining R53 slices (admin UI, switcher, last-owner revoke), CUA gate, `qb-bridge/`, Accounting tab, import UI, Lanvac `fullupdate`, `qb_tasks`. **Order:** finish R53, then CUA, then Windows bridge, then import. R53 slices 1–2 plus the 2026-08-28 audit shipped. R54 read + O5985-gated writes shipped 2026-08-22.

### 4.3 RLS policy matrix

This matrix is the live membership model (R53 slices 1–2). Client access is `private.can_access_profile`. `is_admin()` stays. Later slices add the switcher UI. Delete-Auth / per-site disable live holes were closed in the 2026-08-28 audit; last-owner revoke and multi-site delete confirm copy remain.
...
- [ ] **Multi-site accounts (R53) remaining slices.** Audit of 1–2 is done (Stripe reuse, delete-Auth safety, per-site disable). Still open: last-owner revoke, multi-site delete confirm copy, admin UI, client switcher, emails.
