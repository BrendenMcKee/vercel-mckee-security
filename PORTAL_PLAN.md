...
| `email` | text | unique when set (partial unique index). **Required in the create/edit admin forms** (invitation, Stripe receipts, reminders, sign-in identity). Column stays nullable only for any pre-existing row. Client-locked in Settings (R47); admin may still edit the contact/invite email without changing Auth |
...
| `/api/cron/device-expiry` | via daily / manual | expired devices where `expiry_alerted_at is null` -> email admin always, email client only when mail is live (R52); stamp `expiry_alerted_at` when the admin alert sends (R14) |
...
**Portal-side duplicate guard (independent of QuickBooks).** `profiles.email` is already unique when set (partial unique index), so the same email cannot create two accounts. 8A adds a proactive near-duplicate warning to the create-client flow: an exact email/phone hit or a close name match against existing profiles or unlinked QB mirrors surfaces "a client like this already exists" with a link to their page, before the insert happens. It warns rather than blocks, because two legitimate clients can share a name; the admin decides.
...
2. Committing creates linked `pending` profiles + manual-rail services with true due dates. **No invitations are sent at import.** Re-running the import is idempotent: already-linked mirror rows are skipped, so the seed can never duplicate. **Client-facing mail stays off for the whole import** (9.5.5C): the daily payment-due cron does not email seeded customers either, even when `next_due_on` is inside the reminder window.
3. **The migration then runs in renewal waves off the Billing tab, only after the 8C client-mail flip.** Because every seeded customer's `next_due_on` is real, the collections console covers the entire business from day one, before any client has touched the portal. As each renewal approaches, the admin sends that client's invitation ("activate your account before your next payment"): the client activates, optionally switches to autopay (R29), and their first portal-era payment posts to QuickBooks through 8C. Clients who never activate still get reminder emails (once mail is live), still show on the collections board, and their recorded payments still post to the books: portal activation is a client-experience upgrade, not a bookkeeping prerequisite.
...
- The daily cron flags seeded, never-invited clients whose `next_due_on` enters the reminder window; they surface as an **invite queue** on the Billing tab and in the collections digest, so nobody's renewal slips past without an outreach decision.
- The email for this cohort is a combined **renewal notice + activation invitation**: amount due, due date, legacy payment instructions (paying is never blocked on activation), the client's personal activation link, and the autopay pitch. Sent per-client (or small waves) with one click from the Billing tab.
...
**`portal_settings`** is a singleton (`id = 1`). `client_mail_enabled` defaults **false** and stays false through 8A/8B and the bulk seed. Reads fail closed (missing row or query error = no client mail). A pause is not an `email_failure` alert. `due_alerted_at` is stamped only when a reminder actually sends, so the first reminder after the flip is not skipped.

**Staff mail stays on:** collections digest, card-failed, device-expiry admin, caller-ID admin, account-change admin.

**The flip is an 8C human step, not an import step.** Admin Billing tab: typed confirm `GO LIVE`. Do not turn it on until all four are true:

1. The client import is complete and a sample of accounts is spot-checked.
2. The portal is working the way you expect (billing, contacts, devices).
3. The QuickBooks bridge is on the live company file (`McKee Security Live.QBW`), not PORTAL-TEST (`qb_bridges.mode = live` and `expected_company_file` is the live path).
4. You are ready for customers to receive portal email.

Pause is one confirm and needs no phrase. The same banner shows on every admin tab and on client detail while mail is off.

**Out of this gate (still send):** website contact / apply / inquiry forms, Starlink rental reminders (internal), and Supabase Auth mail (password reset for people who already have an auth user). Activation uses `auth.admin.createUser` and does not send a confirmation email. Marketing and Starlink are live products; they are not the import cohort.
...
- [ ] Create-client near-duplicate warning (9.5.4): exact email/phone or close-name match against existing profiles and unlinked mirrors surfaces "a client like this already exists" before insert (warn, not block)
- [ ] Bulk alignment seed (9.5.5A): "Import from QuickBooks" staging review — drafts prefilled from mirrors (name, email) and invoice history (tier from the billed-amount fingerprint corroborated by line-item names per R41, legacy rates flagged and preserved as billed, `started_on` from the earliest monitoring invoice per R49/D15, projected `next_due_on`), admin confirms/corrects each row, commit creates linked `pending` profiles + manual-rail services, **no invitations sent**; idempotent re-runs (linked rows skipped). `qb_invoices` mirror carries line items for the inference
