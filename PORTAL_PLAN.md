...
|---------|---------|----------------|----------------|
| Security Monitoring | tier + status badge when assigned; if absent, a compact interest/call card is moved below billing and all active-account sections | **none** (read-only; no controls rendered at all); absent clients can call McKee | Handover 6.2: client never changes tier/status. Unused services never appear above services the client actually has (stakeholder 2026-07-18) |
| VoIP Phone Service | plan + rate (x lines when per-line) + status badge (card hidden if client has no VoIP service) | **none** (read-only; line/plan changes go through McKee) | R42: same admin-only management as monitoring; billing rides the shared Billing & Payments card |
| Cloud Backup | tier + status when assigned. Until Track 2 launch, clients without it always see a concise off-site-footage benefit card at the **very bottom** of the dashboard | "Notify Me When Available" explicitly opts the account email into `cloud_backup_interest`; confirmation persists; "Remove me" withdraws. Request Footage arrives in Phase 6B. **No plan controls: assignment/changes/cancellation stay admin-only (R21)** | R43 consent is separate from service assignment. The card explains NVR failure/theft/damage protection without exposing implementation detail |
...
| Client detail | one page per client: profile info, services (assign/modify tier, cancel/restart, set billing method, amount, next due), payment history (both rails), **Camera Cloud Backup interest confirmation (date + consented email, R43)** plus a separate grayed **In Development** retention template (R44; 7/30/90 shown but disabled), devices, caller IDs (view **and edit**, R23; the editor requires an authorization method + reason note before save and shows the diff in the confirm dialog, R24) + change history, invitation state + resend | 3 core, 4 caller ID editor, 5 billing fields |
