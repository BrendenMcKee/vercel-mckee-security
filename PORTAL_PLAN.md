...
**8A first migration slice (applied 2026-08-17 on hosted):** `supabase/migrations/20260817143259_qb_bridge_foundation.sql` (hosted version `20260817143259`). Tables: `qb_bridges`, `qb_customers`, `qb_invoices`, `qb_payments`, `qb_todos`. Alter: `services.started_on date`. RLS: admin SELECT on all five; no client policies; no INSERT/UPDATE/DELETE for `authenticated` (bridge routes use service role). Unique index on `qb_customers.profile_id` where not null. Types regenerated. **Cloud routes (2026-08-17):** `/api/qb/poll`, `/api/qb/report`, `/api/qb/mirror` are live in the website app. **Still not built:** remaining R53 slices (grouping / Appoint / attach, switcher, emails), CUA gate, `qb-bridge/`, Accounting tab, import UI, Lanvac `fullupdate`, `qb_tasks`. **Order:** finish R53, then CUA, then Windows bridge, then import. R53 slices 1–3 shipped; slice 4 first increment shipped 2026-08-30. R54 read + O5985-gated writes shipped 2026-08-22.
...
**Organization grouping (R53, before invitations).** Lanvac Excel has no org id and no email. Seed one pending site per CODE / QB customer. A grouping board on existing profiles (and later on seeded rows) suggests likely groups (exact NAME on 2+ CODEs, civic keywords, same contact email, QB `parent_list_id` when present). Accept / reject only. Never auto-attach. Human sign-off writes `portal_settings.org_grouping_reviewed_at` and is a GO LIVE gate (empty-queue sign-off is allowed so the flip is not blocked before the first import). Attach expires leftover per-site invites and turns `auto_onboard` off. See [`docs/MULTI_SITE_ACCOUNTS.md`](./docs/MULTI_SITE_ACCOUNTS.md).
...
- [ ] **Multi-site accounts (R53) remaining slices.** Still open: grouping board + Appoint account admin + attach/move, client switcher, emails.
