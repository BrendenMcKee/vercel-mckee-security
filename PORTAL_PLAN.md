...
**Next product work (planned):** Multi-site (R53 / [`docs/MULTI_SITE_ACCOUNTS.md`](./docs/MULTI_SITE_ACCOUNTS.md)), then the CUA playbook ([`docs/PORTAL_CUA_TEST.md`](./docs/PORTAL_CUA_TEST.md)) before the Windows bridge or real import. R54 read + O5985-gated writes shipped. Client mail stays off (R52 / 9.5.5C). When a slice changes UI or schema, update those docs in the same commit so implementation is never working from a stale page.
...
**Current identity (pre-R53):** one `profiles` row is the login, the site, and the billing contact. Client RLS is `p.user_id = auth.uid()`. **Planned (R53):** that row stays the site; `accounts` + `account_members` hold the login; client RLS becomes `private.can_access_profile`. Until that ships, this section describes the live schema.
...
| `email` | text | unique when set (partial unique index). **Current (pre-R53).** After multi-site ships, this global unique is dropped so two sites can share a contact email; login identity moves to `account_members`. **Required in the create/edit admin forms** (invitation, Stripe receipts, reminders, sign-in identity). Column stays nullable only for any pre-existing row. Client-locked in Settings (R47); admin may still edit the contact/invite email without changing Auth |
...
**8A first migration slice (applied 2026-08-17 on hosted):** `supabase/migrations/20260817143259_qb_bridge_foundation.sql` (hosted version `20260817143259`). Tables: `qb_bridges`, `qb_customers`, `qb_invoices`, `qb_payments`, `qb_todos`. Alter: `services.started_on date`. RLS: admin SELECT on all five; no client policies; no INSERT/UPDATE/DELETE for `authenticated` (bridge routes use service role). Unique index on `qb_customers.profile_id` where not null. Types regenerated. **Cloud routes (2026-08-17):** `/api/qb/poll`, `/api/qb/report`, `/api/qb/mirror` are live in the website app. **Still not built:** multi-site accounts (R53), CUA gate, `qb-bridge/`, Accounting tab, import UI, Lanvac `fullupdate`, `qb_tasks`. **Order:** R53, then CUA, then Windows bridge, then import. R54 read + O5985-gated writes shipped 2026-08-22.

### 4.3 RLS policy matrix

This matrix is the **live** one-login-per-profile model. R53 replaces client `user_id = auth.uid()` checks with `private.can_access_profile` (membership only). `is_admin()` stays.
...
| `cloud_backup_interest` | none (table grant revoked) | SELECT/INSERT/DELETE own. INSERT also requires the row email to match `profiles.email`; no UPDATE grant. A row is active consent | SELECT all via `private.is_admin()`; no admin write path |
...
**Portal-side duplicate guard (independent of QuickBooks).** **Current (pre-R53):** `profiles.email` is unique when set (partial unique index), so the same email cannot create two client rows. **After R53:** that global unique drops so county-style sites can share a contact email. Login identity is the member email. Create-client still warns on an exact email/phone hit or a close name match against existing **sites**, **account members**, or unlinked QB mirrors, and offers **Add site to an account** when the email already belongs to a member or a site contact. It warns rather than blocks on a close name, because two legitimate clients can share a name; the admin decides. Do not auto-merge.
...
- [ ] **Multi-site accounts (R53).** Locked model in [`docs/MULTI_SITE_ACCOUNTS.md`](./docs/MULTI_SITE_ACCOUNTS.md). Two slices then an audit. First stop: schema + `resolvePortalSession` / orphan / OAuth / cleanup / password. After R54. Do not start the Windows bridge or real import in this item.
