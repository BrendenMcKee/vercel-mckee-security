...
1. Open **Grouping** (`/admin-dashboard?tab=grouping`), or follow the Grouping link on Clients or Billing. Title is **Possible linked accounts**. Click the **(i)** next to **Sign off grouping**. Copy says sign-off is a review gate, does not link sites, and does not send mail. If the queue is empty, that is OK (McKee House + Bunkie already share an account). You may click **Sign off grouping**. **Do not treat empty as a crash.**
