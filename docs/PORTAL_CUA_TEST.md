...
2. `?tab=clients`: list of clients. Search by a known test name. Open that row. House and Bunkie (or any two-site test account) each show the same amber account chip (`McKee · 2 sites` or the account name and a count greater than 1). Their emails may tint amber. A single-site house must **not** show a site-count chip. Filter **Linked accounts** keeps only those rows. Filter **One site** hides them. Search by the account name (McKee) finds both. Do not call this enterprise.
