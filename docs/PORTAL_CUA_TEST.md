...
2. Welcome uses their first name. Under it, **site display name · CODE** (or No CODE). An Account line is OK when the account name differs or there are 2+ sites. That line is not a switcher. One monitoring (and optional VoIP) card. No site switcher. No sites list. No “People with access.”
...
6. Security tab: the header still shows site name · CODE. The Security card subtitle may also start with the CODE. Caller list visible if contacts exist. Edit an existing person (name / phone / passcode) or save a harmless reorder. Confirm the list is this house only. Dashboard must not show the list or pull Lanvac.
...
2. Open **New client**. Type first/last that match an existing site (Brenden McKee) or the McKee contact email. An amber **Possible linked account** card appears. Click **More info**. Copy says it does not attach. First and last name are the site label (Dysart / Library for a town site). **Add site to McKee** (or the named account) switches mode. Then Close, or change the fields to a unique `cua-house-{time}@example.com` before the next step. A civic-looking name (Library, County) is a review flag only. Do not treat it as a one-click merge.
3. **New client:** one first name, last name, email (`cua-house-{time}@example.com`), optional address/phone, one monitoring tier, a unique fake CODE (follow the on-screen CODE rules), a dispatch city, billing rail Manual. Submit.
4. Success: client exists. Invite is held or a copy-link is shown. You are not told that mail went out.
5. Open the new client. One site. No sibling-site list (or a count of 1). Account card may exist; org chrome on the **client** side must still be hidden when you later log in as them (Suite B style) if they are the only person and only site.
6. **Add site to an account:** pick the CUA County account (or the house you just created if the operator wants a two-site test). Enter a second site name, a **different** CODE, same or other city, monitoring, Manual rail. Submit.
7. No second invite email. Account now has two sites. `auto_onboard` / auto-invite is off or the UI says invites are not automatic. The added site has **No invite** and no Resend (it must not mint a second house invite). The original site may still show Resend if it has an open invite.
8. Clients list: the two county rows chip the same account name and a site count greater than 1. Linked accounts filter keeps both. Search by that account name finds both.
9. Open site A. Account card lists site B as a link. Link works. Delete is labeled as this **site** if a delete control exists. **Do not delete** unless the operator said the row is disposable.
10. Disable site B only. Site A still opens. Re-enable site B. If re-enable is blocked because `user_id` is empty, that is a fail (plan item 18).
11. New client again with an email that already belongs to the county account admin. Warning must offer **Add site to an account**, not a silent second login.

## Suite F. Grouping board and Appoint account admin

1. Open **Grouping** (`/admin-dashboard?tab=grouping`), or follow the Grouping link on Clients or Billing. Title is **Possible linked accounts**. Click **More info** next to **Sign off grouping**. Copy says why the stamp exists, that it does not link sites, and that it does not send mail. If the queue is empty, that is OK (McKee House + Bunkie already share an account). You may click **Sign off grouping**. **Do not treat empty as a crash.**
2. If a suggestion exists for two CUA test rows (`@example.com`, fake CODEs), Accept or Reject only those rows. Uncheck a site to leave it out. Confirm says it does not send email. Do not accept a suggestion that would merge McKee House / Bunkie or other real-looking names.
...
1. `/user-dashboard` shows a **sites list** (name, address, CODE, short caller-list summary) and a switcher. The header still shows the selected **site name · CODE**. Single-site chrome from Suite B must not be the only view. This suite fails until slice 5 ships. That is expected today.
