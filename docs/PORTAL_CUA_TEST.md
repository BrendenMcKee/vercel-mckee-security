...
2. `?tab=clients`: list of clients. Search by a known test name. Open that row. House and Bunkie (or any two-site test account) share an **Account** column chip (`McKee · 2 sites` or the account name and a count greater than 1). Name and email stay ordinary. A single-site house has a dash in Account, not a site-count chip. Filter **Linked accounts** keeps only those rows. Filter **One site** hides them. Search by the account name (McKee) finds both. Do not call this enterprise.
