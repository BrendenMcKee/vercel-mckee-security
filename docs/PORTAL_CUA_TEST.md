...
5. Staff client detail **Security tab**: header names the Lanvac CODE and city. Monitoring station, Zones & Signals (red cards), Refresh now, Historic, one Zones table (Add / Delete; Edit only after a portal create), and **account** on-test. No zone on-test buttons. Create-client does not require zones. Carbon monoxide type cannot be changed yet. Pulled live zones have no Edit. Account OffTest can return 500 if already off; the UI should still succeed. A refresh must not clear the account chip (`on_test_until` is the SoR).
