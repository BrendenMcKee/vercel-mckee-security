...
**12. Site cookie / `?site=`.** Only honor a site the member can access and that is not disabled. Otherwise the leftover home site (`profiles.user_id` for this login) if it is active, else first active site. A crafted id is not an IDOR. The Next.js `proxy` copies a valid `?site=` UUID onto the cookie and `x-portal-selected-site` so the layout and server actions match the page on the same request.
...
**24. Admin list and digest noise.** **Clients list chip + filter shipped 2026-08-30.** Chip the account name and site count when the account has two or more sites. Search by account name. Filter Linked accounts vs One site. Collections digest should group by account so 14 county lines are not 14 unrelated people. “Activated” means the **account** has an activated owner, not “this row has `user_id`.”
...
- `portal_settings.org_grouping_reviewed_at` (nullable). [`setClientMailEnabledAction`](website/src/lib/portal/actions/settings.ts) refuses GO LIVE until it is set **and** no suggestion is still `open`. Sign-off is allowed when the queue is **empty**. A new open suggestion clears the timestamp.
- `account_group_suggestions` + `account_group_suggestion_sites` (admin RLS). Fingerprint is `kind` plus sorted profile ids. Rejected fingerprints do not come back. Civic keyword names are a watchlist only (no one-click merge). Accept uses [`grouping-attach.ts`](../website/src/lib/portal/grouping-attach.ts) (reuse this for Account-card attach/move). If the set already includes one multi-site account, the suggested name stays that account name so Accept does not rename McKee or the county.
...
4. Admin: New client / Add site + Account card **shipped 2026-08-30**, UI polish **signed off 2026-09-05**. Grouping board + empty-queue sign-off **shipped 2026-09-07**. Appoint account admin and Account-card attach/move still open. **This is the next implementation.**
