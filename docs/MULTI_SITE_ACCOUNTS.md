...
- **Every client header (shipped 2026-09-12):** Welcome first name, then **site display name · CODE** (or No CODE). An Account line appears when the account name differs from the site name or the account has two or more sites. This is identity only. It is not a switcher.
- **One site and one member:** today’s dashboard. No switcher, no sites list, no “people with access.”
- **Two or more sites:** a **sites list** (name, address, CODE, short caller-list summary) so they can see every system without opening 14 logins, plus a switcher (**slice 5, not built yet**). Clicking a row selects that site. Billing, the full caller-ID editor, VoIP, and devices stay **one site at a time** (one CODE, one Stripe customer, one VoIP). Do not build one combined editable call-list for every site on one form (easy to save the wrong list).
