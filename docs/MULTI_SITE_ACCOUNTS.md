...
Clients list: **shipped 2026-08-30.** An **Account** column chips the account name and site count when that account has two or more sites (example: `McKee · 2 sites`). Single-site rows show a dash. Name and email stay ordinary. The link key is `account_id`, not a repeated site-contact email. Search matches account name. Filter: All sites / Linked accounts / One site. Do not say enterprise, organization, or Master.
