...
> **Current implementation is in [`PORTAL_PLAN.md`](./PORTAL_PLAN.md) and the code.** This handover is the July 2026 requirements baseline (what / why). Several UX decisions have since superseded the sketches here: the client portal is tabbed (Dashboard / Security / Settings / Alerts), sign-in email is locked, devices are an open categorized list (not a fixed battery/smoke pair), caller ID is editable by admin with recorded authorization, VoIP is a first-class service, QuickBooks accounting is Track 1 Phase 8, and **client-facing portal mail stays off until the 8C `GO LIVE` flip** (`PORTAL_PLAN.md` R52 / 9.5.5C). **Lanvac station layer (R54):** read UI and O5985-gated writes / on-test shipped (`docs/LANVAC_STATION.md`). Showing who clicked on-test in the portal is R54b (later). Per-staff Lanvac passwords are R54c (later). **Multi-site accounts (R53 slices 1–3 shipped; slice 4 first increment + 2026-09-05 UI polish shipped):** one account can have many sites and extra logins (`docs/MULTI_SITE_ACCOUNTS.md`). Staff Clients list has two red **New client** / **Add site to an account** buttons (toggle only after a form is open), plus an Account card (this-site first, auto-onboard). Grouping board shipped 2026-09-07. Staff More info help, create-form grouping hint, and client header site · CODE shipped 2026-09-12. Remaining: Appoint / attach, client switcher, emails. Then run [`docs/PORTAL_CUA_TEST.md`](./docs/PORTAL_CUA_TEST.md) before the Windows bridge or real import. Technician mobile + Alarmnet are R55 (after GO LIVE). Do not implement from this document's "four main sections" layout without checking the plan.
...
Admin creates pending client profile + assigns tiers
        ↓
System sends invite email once client mail is live (held until GO LIVE; multi-site orgs get one Account admin setup letter, not one invite per site)
        ↓
